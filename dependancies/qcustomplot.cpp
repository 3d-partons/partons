...
QCPItemPosition::QCPItemPosition(QCustomPlot *parentPlot,
        ...
        : QCPItemAnchor(parentPlot, parentItem, name), mPositionTypeX(
                ptAbsolute), mPositionTypeY(ptAbsolute), mKey(0), mValue(0), mParentAnchorX(
                0), mParentAnchorY(0) {
}
...
QCPColorScaleAxisRectPrivate::QCPColorScaleAxisRectPrivate(
    ...
    foreach (QCPAxis::AxisType type, QList<QCPAxis::AxisType>() << QCPAxis::atBottom << QCPAxis::atTop << QCPAxis::atLeft << QCPAxis::atRight)connect(parentColorScale, SIGNAL(layerChanged(QCPLayer*)), axis(type), SLOT(setLayer(QCPLayer*)));
}
 ...
 */
QCPData::QCPData()
        : key(0), value(0), keyErrorPlus(0), keyErrorMinus(0), valueErrorPlus(
                0), valueErrorMinus(0) {
}
...
double QCPGraph::pointDistance(const QPointF &pixelPoint) const {
    ...
    if (mLineStyle == lsNone) {
        ...
        double minDistSqr = std::numeric_limits<double>::max();
        QPointF ptA;
        QPointF ptB = coordsToPixels(scatterData->at(0).key,
        ...
        for (int i = 1; i < scatterData->size(); ++i) {
            ptA = ptB;
            ptB = coordsToPixels(scatterData->at(i).key,
...
QCPBarsGroup::QCPBarsGroup(QCustomPlot *parentPlot)
        : QObject(parentPlot), mParentPlot(parentPlot), mSpacingType(
                stAbsolute), mSpacing(4) {
}
...
QCPFinancial::QCPFinancial(QCPAxis *keyAxis, QCPAxis *valueAxis)
        : QCPAbstractPlottable(keyAxis, valueAxis), mData(0), mChartStyle(
                csOhlc), mWidth(0.5), mTwoColored(false), mBrushPositive(
