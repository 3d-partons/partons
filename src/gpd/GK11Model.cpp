...
GK11Model::GK11Model() :
    ...
    init();
    std::cerr << "[GK11Model] default constructor | id = " << getId()
            << std::endl;
}
...
GK11Model::GK11Model(const GK11Model& other) :
    ...
    std::cerr << "[GK11Model] copy constructor| id = " << getId() << std::endl;
}
...
GPDResultData GK11Model::computeH(const double &_x, const double &_xi,
        const double &_t) {
                    ...
                    * (2. * gpdQuarkFlavorData_u.getSea()
                    ...
                    + D2_ELEC_CHARGE
                            * (2. * gpdQuarkFlavorData_d.getSea()
                                    + gpdQuarkFlavorData_d.getValence()
                                    - fHdValMx)
                    + S2_ELEC_CHARGE * 2. * gpdQuarkFlavorData_s.getHq()); // True result !
...
GPDResultData GK11Model::computeHt(const double &_x, const double &_xi,
        const double &_t) {
    ...
    GPD_Ht.setSinglet(
            U2_ELEC_CHARGE * (gpdQuark_u.getSinglet())
                    + D2_ELEC_CHARGE * (gpdQuark_d.getSinglet()));
...
GPDResultData GK11Model::computeE(const double &_x, const double &_xi,
        const double &_t) {
    ...
    GPD_E.setGluon(exp(b0 * _t) * (c1 * Ei1tab.at(0) + c2 * Ei1tab.at(1)));
    ...
    GPD_E.setSinglet(
            U2_ELEC_CHARGE * gpdQuarkFlavorData_u.getSinglet()
                    + D2_ELEC_CHARGE * gpdQuarkFlavorData_d.getSinglet()
                    + S2_ELEC_CHARGE * gpdQuarkFlavorData_s.getSinglet());
...
GPDResultData GK11Model::computeEt(const double &_x, const double &_xi,
        const double &_t) {
