...
int ConvolCoeffFunctionResultDao::insert(double realPart, double imgPart,
        int gpdType, int kinematicId, int computationId) const {
    int result = -1;
    QSqlQuery query(DatabaseManager::getInstance()->getDb());

    query.prepare(
            "INSERT INTO convol_coeff_function_result (real_part, img_part, gpd_type_id, convol_coeff_function_kinematic_id, computation_id) VALUES (:realPart, :imgPart, :gpdType, :kinematicId, :computationId)");
