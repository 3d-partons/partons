...
BCVertex::BCVertex(const std::string& className,
        ...
        QuarkGluonVertex(className, numberOfBasisElements) {
}
...
std::vector<double> BCVertex::Angular_Integrands(double p2, double q2,
        double k2) const {
    std::vector<double> angular_integrands(2 * m_N, 0.);
    //TODO Add tests before each "at.(i)" access to see if it's smaller than m_N
    // (to allow such things as BCSimplfiedVertex inheriting from BCvertex)
    //TODO Optimize code
...
std::vector<double> BCVertex::Radial_Integrands(double p2, double q2,
    ...
    radial_integrands.at(m_N + 1) = sigmaS_q2 * (A_p2 - A_q2);
    radial_integrands.at(m_N + 2) = sigmaV_q2 * (B_p2 - B_q2);
...
std::vector<double> BCVertex::Radial_Integrands_deriv(double p2, double q2,
    ...
    // (to allow such things as BCSimplfiedVertex inheriting from BCvertex)
    //TODO Optimize code
    radial_integrands_deriv.at(0) = (dsigmaV_q2 * (A_p2 + A_q2)
            + sigmaV_q2 * (dA_p2 + dA_q2)) * 0.5;
    radial_integrands_deriv.at(1) = dsigmaV_q2 * (A_p2 - A_q2)
            + sigmaV_q2 * (dA_p2 - dA_q2);
    radial_integrands_deriv.at(2) = dsigmaS_q2 * (B_p2 - B_q2)
            + sigmaS_q2 * (dB_p2 - dB_q2);
    radial_integrands_deriv.at(m_N) = (dsigmaS_q2 * (A_p2 + A_q2)
            + sigmaS_q2 * (dA_p2 + dA_q2)) * 0.5;
    radial_integrands_deriv.at(m_N + 1) = dsigmaS_q2 * (A_p2 - A_q2)
            + sigmaS_q2 * (dA_p2 - dA_q2);
    radial_integrands_deriv.at(m_N + 2) = dsigmaV_q2 * (B_p2 - B_q2)
