...
TCSObservableResultDaoService::TCSObservableResultDaoService() :
        ObservableResultDaoService("TCSObservableResultDaoService") {
    ...
    if (query.exec(
            "SELECT COUNT(tcs_observable_kinematic_id) FROM tcs_observable_kinematic")) {
        if (query.first()) {
    ...
    if (query.exec(
            "SELECT COUNT(tcs_observable_result_id) FROM tcs_observable_result")) {
        if (query.first()) {
...
int TCSObservableResultDaoService::insert(
        const TCSObservableResult &observableResult) {
...
int TCSObservableResultDaoService::insert(
        const List<TCSObservableResult> &resultList) {
    ...
    try {
        ...
        for (unsigned int i = 0; i != resultList.size(); i++) {
            ...
            //get kinematics
            TCSObservableKinematic kinematic = resultList[i].getKinematic();
        ...
        insertDataIntoDatabaseTables("observableKinematicTableFile.csv",
                m_observableKinematicTableFile, "tcs_observable_kinematic");
        insertDataIntoDatabaseTables("observableResultTableFile.csv",
                m_observableResultTableFile, "tcs_observable_result");
...
List<TCSObservableResult> TCSObservableResultDaoService::getObservableResultListByComputationId(
        const int computationId) const {
    return m_observableResultDao.getObservableResultListByComputationId(
            computationId);
}

List<TCSObservableResult> TCSObservableResultDaoService::getObservableResultListFromSQLQuery(
