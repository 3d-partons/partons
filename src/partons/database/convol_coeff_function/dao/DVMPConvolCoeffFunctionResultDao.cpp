#include "../../../../../include/partons/database/convol_coeff_function/dao/DVMPConvolCoeffFunctionResultDao.h"
...
DVMPConvolCoeffFunctionResultDao::DVMPConvolCoeffFunctionResultDao() :
        ConvolCoeffFunctionResultDao("DVMPConvolCoeffFunctionResultDao") {
}

DVMPConvolCoeffFunctionResultDao::~DVMPConvolCoeffFunctionResultDao() {
}

int DVMPConvolCoeffFunctionResultDao::insertIntoDVMPCCFResultComplex(
    ...
    //prepare query
    query.prepare(
            "INSERT INTO dvmp_ccf_result (real_part, img_part, gpd_type_id, ccf_result_id) VALUES (:realPart, :imgPart, :gpdTypeId, :ccfResultId)");
...
List<DVMPConvolCoeffFunctionResult> DVMPConvolCoeffFunctionResultDao::getResultListByComputationId(
    ...
    //prepare query
    query.prepare(
            "SELECT * FROM dvmp_ccf_result_view WHERE computation_id = :computationId;");
...
void DVMPConvolCoeffFunctionResultDao::fillConvolCoeffFunctionResultList(
        List<DVMPConvolCoeffFunctionResult> &resultList,
    ...
    //results
    DVMPConvolCoeffFunctionResult ccfResult;
    ...
    if (query.first()) {
        ...
        ccfResult.setKinematic(
                m_dvmpConvolCoeffFunctionKinematicDao.getKinematicById(
    ...
    while (query.next()) {
        ...
        if (currentCCFResultId != ccfResult.getIndexId()) {
            ...
            ccfResult = DVMPConvolCoeffFunctionResult();
            ccfResult.setIndexId(currentCCFResultId);
