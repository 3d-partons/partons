#include "../../../../../include/partons/database/convol_coeff_function/dao/DDVCSConvolCoeffFunctionResultDao.h"
...
DDVCSConvolCoeffFunctionResultDao::DDVCSConvolCoeffFunctionResultDao() :
        ConvolCoeffFunctionResultDao("DDVCSConvolCoeffFunctionResultDao") {
}

DDVCSConvolCoeffFunctionResultDao::~DDVCSConvolCoeffFunctionResultDao() {
}

int DDVCSConvolCoeffFunctionResultDao::insertIntoDDVCSCCFResultComplex(
    ...
    //prepare query
    query.prepare(
            "INSERT INTO ddvcs_ccf_result (real_part, img_part, gpd_type_id, ccf_result_id) VALUES (:realPart, :imgPart, :gpdTypeId, :ccfResultId)");
...
List<DDVCSConvolCoeffFunctionResult> DDVCSConvolCoeffFunctionResultDao::getResultListByComputationId(
    ...
    //prepare query
    query.prepare(
            "SELECT * FROM ddvcs_ccf_result_view WHERE computation_id = :computationId;");
...
void DDVCSConvolCoeffFunctionResultDao::fillConvolCoeffFunctionResultList(
        List<DDVCSConvolCoeffFunctionResult> &resultList,
    ...
    //results
    DDVCSConvolCoeffFunctionResult ccfResult;
    ...
    if (query.first()) {
        ...
        ccfResult.setKinematic(
                m_ddvcsConvolCoeffFunctionKinematicDao.getKinematicById(
    ...
    while (query.next()) {
        ...
        if (currentCCFResultId != ccfResult.getIndexId()) {
            ...
            ccfResult = DDVCSConvolCoeffFunctionResult();
            ccfResult.setIndexId(currentCCFResultId);
