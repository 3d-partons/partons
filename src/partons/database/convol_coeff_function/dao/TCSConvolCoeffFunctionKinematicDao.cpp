#include "../../../../../include/partons/database/convol_coeff_function/dao/TCSConvolCoeffFunctionKinematicDao.h"
...
TCSConvolCoeffFunctionKinematicDao::TCSConvolCoeffFunctionKinematicDao() :
        ConvolCoeffFunctionKinematicDao("TCSConvolCoeffFunctionKinematicDao") {
}

TCSConvolCoeffFunctionKinematicDao::~TCSConvolCoeffFunctionKinematicDao() {
}

int TCSConvolCoeffFunctionKinematicDao::insert(const PhysicalType<double>& xi,
    ...
    //prepare query
    query.prepare(
            "INSERT INTO tcs_ccf_kinematic (xi, xi_unit, t, t_unit, Q2Prim, Q2Prim_unit, MuF2, MuF2_unit, MuR2, MuR2_unit, hash_sum) VALUES (:xi, :xi_unit, :t, :t_unit, :Q2Prim, :Q2Prim_unit, :MuF2, :MuF2_unit, :MuR2, :MuR2_unit, :hash_sum);");
...
int TCSConvolCoeffFunctionKinematicDao::select(const PhysicalType<double>& xi,
        const PhysicalType<double>& t, const PhysicalType<double>& Q2Prim,
    ...
    //prepare query
    query.prepare(
            "SELECT tcs_ccf_kinematic_id FROM tcs_ccf_kinematic WHERE xi = :xi AND xi_unit = :xi_unit AND t = :t AND t_unit = :t_unit AND Q2Prim = :Q2Prim AND Q2Prim_unit = :Q2Prim_unit AND MuF2 = :MuF2 AND MuF2_unit = :MuF2_unit AND MuR2 = :MuR2 AND MuR2_unit = :MuR2_unit;");
...
TCSConvolCoeffFunctionKinematic TCSConvolCoeffFunctionKinematicDao::getKinematicById(
        const int id) const {

    //result
    TCSConvolCoeffFunctionKinematic convolCoeffFunctionKinematic;
    ...
    //prepare query
    query.prepare(
            "SELECT * FROM tcs_ccf_kinematic_view WHERE tcs_ccf_kinematic_id = :id;");
...
List<TCSConvolCoeffFunctionKinematic> TCSConvolCoeffFunctionKinematicDao::getKinematicListByComputationId(
...
void TCSConvolCoeffFunctionKinematicDao::fillKinematicListFromQuery(
        List<TCSConvolCoeffFunctionKinematic>& kinematicList,
...
void TCSConvolCoeffFunctionKinematicDao::fillKinematicFromQuery(
        TCSConvolCoeffFunctionKinematic &kinematic, QSqlQuery &query) const {

    //get indices
    int field_id = query.record().indexOf("tcs_ccf_kinematic_id");
...
int TCSConvolCoeffFunctionKinematicDao::getKinematicIdByHashSum(
        const std::string& hashSum) const {
    ...
    //prepare query
    query.prepare(
            "SELECT tcs_ccf_kinematic_id FROM tcs_ccf_kinematic WHERE hash_sum = :hashSum;");
