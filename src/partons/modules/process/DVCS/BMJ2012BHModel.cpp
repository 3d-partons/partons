#include "../../../../../include/partons/modules/process/DVCS/BMJ2012BHModel.h"
...
const unsigned int BMJ2012BHModel::classId =
        BaseObjectRegistry::getInstance()->registerBaseObject(
                new BMJ2012BHModel("BMJ2012BHModel"));

/*--------------------------------------- Constructors ---------------------------------*/

BMJ2012BHModel::BMJ2012BHModel(const std::string &className) :
...
BMJ2012BHModel::~BMJ2012BHModel() {
}

BMJ2012BHModel::BMJ2012BHModel(const BMJ2012BHModel& other) :
        DVCSModule(other) {
...
BMJ2012BHModel* BMJ2012BHModel::clone() const {
    return new BMJ2012BHModel(*this);
}

void BMJ2012BHModel::initModule() {
    ...
    m_phaseSpace = m_xB * m_yBMJ[1]
            / (1024 * pow(PI, 5) * m_Q[3] * m_epsroot[0]);

}

void BMJ2012BHModel::initModule(double beamHelicity, double beamCharge,
...
void BMJ2012BHModel::isModuleWellConfigured() {

    //check mother class
    DVCSModule::isModuleWellConfigured();
}

void BMJ2012BHModel::defineAngles(const NumA::Vector3D &targetPolarization) {
...
void BMJ2012BHModel::computeFormFactors() {
...
void BMJ2012BHModel::computeCFFs() {
...
void BMJ2012BHModel::computeFourierCoeffsBH() {
    //TODO Get the value of F1(t) and F2(t) from another module
...
void BMJ2012BHModel::computeFourierCoeffsVCS() {
    double C1, C2;
...
void BMJ2012BHModel::computeAngularCoeffsInterf() {
    double tQ2 = m_t / m_Q2;
...
void BMJ2012BHModel::computeFourierCoeffsInterf() {
// Unpolarized Fourier coeffs
...
std::complex<double> BMJ2012BHModel::CFF(GPDType::Type F, int a, int b) {
    unsigned int j;
...
std::complex<double> BMJ2012BHModel::C_VCS(unsigned int S, int a1, int b1,
        int a2, int b2) {
...
std::complex<double> BMJ2012BHModel::C_I(unsigned int S, int a, int b,
        const std::string& VA) {
...
std::complex<double> BMJ2012BHModel::C_I(unsigned int S, unsigned int n, int a,
        int b) {
    unsigned int i;
    if (a == -1) {
        a = 1;
...
std::complex<double> BMJ2012BHModel::S_I(unsigned int S, unsigned int n, int a,
        int b) {
    unsigned int i;
    if (a == -1) {
        a = 1;
...
double BMJ2012BHModel::SqrAmplBH(double beamHelicity, double beamCharge,
        NumA::Vector3D targetPolarization) {
...
double BMJ2012BHModel::SqrAmplVCS(double beamHelicity, double beamCharge,
        NumA::Vector3D targetPolarization) {
...
double BMJ2012BHModel::SqrAmplInterf(double beamHelicity, double beamCharge,
        NumA::Vector3D targetPolarization) {
...
double BMJ2012BHModel::CrossSectionBH(double beamHelicity, double beamCharge,
        NumA::Vector3D targetPolarization) {
    return SqrAmplBH(beamHelicity, beamCharge, targetPolarization)
            * m_phaseSpace;
}

double BMJ2012BHModel::CrossSectionVCS(double beamHelicity, double beamCharge,
        NumA::Vector3D targetPolarization) {
    return 0;
}
