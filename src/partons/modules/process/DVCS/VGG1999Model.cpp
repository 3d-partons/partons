...
double VGG1999Model::CrossSectionMechanism(double beamHelicity,
    ...
    //longitudinaly polarised target
    m_proton_spinor_orientation = z;

    plus = v_compton_doublepol_cross_inv(beamCharge, mechanism, beamHelicity,
            0.5);
    minus = v_compton_doublepol_cross_inv(beamCharge, mechanism, beamHelicity,
            -0.5);

    result += 0.5 * (1. + m_target_polarizationL) * plus
            + 0.5 * (1. - m_target_polarizationL) * minus;
    ...
    printf("PROCESS: %d RESULT: %f\n", mechanism, nbToGeVm2conversion(result));
...
std::complex<double> VGG1999Model::AmplitudeBH(int mu, double el_hel,
    ...
    //outgoing photon virtuality
    double qp_sqr = V4mul(q4_out, q4_out).real();
...
std::complex<double> VGG1999Model::v_compton_t_matrix_hel(double leptcharge,
    ...
    case 3: {
        return (AmplitudeBH(mu, el_hel, sp_in, sp_out)
                + (leptcharge * AmplitudeDVCS(mu, el_hel, sp_in, sp_out)));
    }
...
double VGG1999Model::v_compton_doublepol_sqrampl(double leptcharge,
    ...
    double t_incr;
    std::complex<double> t_matrix_mu;

    for (double sp_out = -.5; sp_out <= .5; sp_out += 1.) {

        std::complex<double> t_matrix_x = std::complex<double>(0., 0.);
        std::complex<double> t_matrix_y = std::complex<double>(0., 0.);
        std::complex<double> t_matrix_0 = std::complex<double>(0., 0.);

        for (int mu = 0; mu <= 3; mu++) {

            t_matrix_mu = v_compton_t_matrix_hel(leptcharge, mechanism, mu,
                    el_hel, sp_in, sp_out);
        ...
        t_incr = (((t_matrix_x * std::conj(t_matrix_x))
                + (t_matrix_y * std::conj(t_matrix_y)))
                + (t_matrix_0 * std::conj(t_matrix_0))).real();
