...
DDVCSProcessDMSW22::DDVCSProcessDMSW22(const std::string &className) :
    ...
    for (i = 0; i < 4; i++) {
        ...
        m_DMSW_p[i] = 0.;
        m_DMSW_pPrim[i] = 0.;
        m_DMSW_pPrim_tMin[i] = 0.;
        m_DMSW_k[i] = 0.;
        m_DMSW_k_tMin[i] = 0.;
        m_DMSW_Delta[i] = 0.;
        m_DMSW_qPrim[i] = 0.;
        m_DMSW_q[i] = 0.;
        m_DMSW_qPrim_tMin[i] = 0.;
        m_DMSW_q_tMin[i] = 0.;
        m_DMSW_kPrime[i] = 0.;
        m_DMSW_lminus[i] = 0.;
        m_DMSW_lplus[i] = 0.;
...
DDVCSProcessDMSW22::DDVCSProcessDMSW22(const DDVCSProcessDMSW22& other) :
    ...
    m_DMSW_Q2Bar = other.m_DMSW_Q2Bar;
    m_DMSW_Q2Bar_tMin = other.m_DMSW_Q2Bar_tMin;
    m_DMSW_cosTheta_e = other.m_DMSW_cosTheta_e;
    ...
    for (i = 0; i < 4; i++) {
        m_DMSW_pPrim[i] = other.m_DMSW_pPrim[i];
        m_DMSW_pPrim_tMin[i] = other.m_DMSW_pPrim_tMin[i];
    }
    ...
    for (i = 0; i < 4; i++) {
        ...
        m_DMSW_qPrim[i] = other.m_DMSW_qPrim[i];
        m_DMSW_q[i] = other.m_DMSW_q[i];
        m_DMSW_qPrim_tMin[i] = other.m_DMSW_qPrim_tMin[i];
        m_DMSW_q_tMin[i] = other.m_DMSW_q_tMin[i];
    ...
    m_DMSW_rho = other.m_DMSW_rho;
    m_DMSW_rho_tMin = other.m_DMSW_rho_tMin;
    m_DMSW_y = other.m_DMSW_y;
    m_DMSW_2pBarqBar = other.m_DMSW_2pBarqBar;
    m_DMSW_2pBarqBar_tMin = other.m_DMSW_2pBarqBar_tMin;
...
void DDVCSProcessDMSW22::initModule() {
    ...
    computeInternalVariables(Constant::PROTON_MASS);
...
PhysicalType<double> DDVCSProcessDMSW22::CrossSectionBH() {
    ...
    if (m_targetPolarization.getX() == 0. && m_targetPolarization.getY() == 0.
            && m_targetPolarization.getZ() == 0.) { //unpolarized target
        targetPolariz = 0;
    } else if (m_targetPolarization.getX() == 0.
            && m_targetPolarization.getY() == 0.
            && m_targetPolarization.getZ() != 0.) { // longitudinally polarized target: collinear (m_targetPolarization.getZ() > 0), or counter-collinear (m_targetPolarization.getZ() < 0) to the incoming electron beam)
        targetPolariz = m_targetPolarization.getZ()
                / fabs(m_targetPolarization.getZ());
    } else {
        throw ElemUtils::CustomException(getClassName(), __func__,
                ElemUtils::Formatter()
                        << "Not able to run for TRANSVERSE target polarization "
...
std::complex<double> DDVCSProcessDMSW22::ampliBH1(int s2, int s1, int sl,
    ...
    int i, j;
    for (i = 0; i < 4; i++) {
        L[0][i] = m_DMSW_lminus[i];
        L[1][i] = m_DMSW_lplus[i];
        L[2][i] = m_DMSW_kPrime[i];
    }
    ...
    for (j = 0; j < 3; j++) {
        tBH1_J1 += fFunction(sl, m_DMSW_lminus, m_DMSW_lplus, s, m_DMSW_kPrime,
                L[j])
                * (Yfunction(s2, s1)
                        * fFunction(s, L[j], m_DMSW_k, +1, rPrime[h2], r[h1])
                        + Zfunction(s2, s1)
                                * fFunction(s, L[j], m_DMSW_k, -1,
    ...
    for (j = 0; j < 3; j++) {
        for (i = 0; i < 2; i++) {
            ...
            tBH1_J2 += fFunction(sl, m_DMSW_lminus, m_DMSW_lplus, s,
                    m_DMSW_kPrime, L[j])
                    * gFunction(s, L[j], rPrime[i], m_DMSW_k);
            tBH1_J2 += fFunction(sl, m_DMSW_lminus, m_DMSW_lplus, s,
                    m_DMSW_kPrime, L[j]) * gFunction(s, L[j], r[i], m_DMSW_k);
...
std::complex<double> DDVCSProcessDMSW22::ampliBH1crossed(int s2, int s1, int sl,
    ...
    for (j = 0; j < 3; j++) {
        tBH1_J1 += sigma[j]
                * fFunction(sl, m_DMSW_lminus, m_DMSW_lplus, s, L[j], m_DMSW_k)
                * (Yfunction(s2, s1)
                        * fFunction(s, m_DMSW_kPrime, L[j], +1, rPrime[h2],
                                r[h1])
                        + Zfunction(s2, s1)
                                * fFunction(s, m_DMSW_kPrime, L[j], -1,
                                        rPrime[minush2], r[minush1]));
    }
    ...
    for (j = 0; j < 3; j++) {
        for (i = 0; i < 2; i++) {
            ...
            tBH1_J2 += sigma[j]
                    * fFunction(sl, m_DMSW_lminus, m_DMSW_lplus, s, L[j],
                            m_DMSW_k)
                    * gFunction(s, m_DMSW_kPrime, rPrime[i], L[j]);
            tBH1_J2 += sigma[j]
                    * fFunction(sl, m_DMSW_lminus, m_DMSW_lplus, s, L[j],
                            m_DMSW_k) * gFunction(s, m_DMSW_kPrime, r[i], L[j]);
...
std::complex<double> DDVCSProcessDMSW22::ampliBH2(int s2, int s1, int sl,
    ...
    int i, j;
    for (i = 0; i < 4; i++) {
        L[0][i] = m_DMSW_kPrime[i];
        L[1][i] = m_DMSW_lminus[i];
        L[2][i] = m_DMSW_k[i];
    }
    ...
    for (j = 0; j < 3; j++) {
        tBH2_J1 += sigma[j]
                * fFunction(sl, m_DMSW_lminus, L[j], s, m_DMSW_kPrime, m_DMSW_k)
                * (Yfunction(s2, s1)
                        * fFunction(sl, L[j], m_DMSW_lplus, +1, rPrime[h2],
                                r[h1])
                        + Zfunction(s2, s1)
                                * fFunction(sl, L[j], m_DMSW_lplus, -1,
                                        rPrime[minush2], r[minush1]));
    }
    ...
    for (j = 0; j < 3; j++) {
        for (i = 0; i < 2; i++) {
            ...
            tBH2_J2 += sigma[j]
                    * fFunction(sl, m_DMSW_lminus, L[j], s, m_DMSW_kPrime,
                            m_DMSW_k)
                    * gFunction(sl, L[j], rPrime[i], m_DMSW_lplus);
            tBH2_J2 += sigma[j]
                    * fFunction(sl, m_DMSW_lminus, L[j], s, m_DMSW_kPrime,
                            m_DMSW_k) * gFunction(sl, L[j], r[i], m_DMSW_lplus);
...
std::complex<double> DDVCSProcessDMSW22::ampliBH2crossed(int s2, int s1, int sl,
    ...
    for (j = 0; j < 3; j++) {
        tBH2_J1 += sigma[j]
                * fFunction(sl, L[j], m_DMSW_lplus, s, m_DMSW_kPrime, m_DMSW_k)
                * (Yfunction(s2, s1)
                        * fFunction(sl, m_DMSW_lminus, L[j], +1, rPrime[h2],
                                r[h1])
                        + Zfunction(s2, s1)
                                * fFunction(sl, m_DMSW_lminus, L[j], -1,
                                        rPrime[minush2], r[minush1]));
    }
    ...
    for (j = 0; j < 3; j++) {
        for (i = 0; i < 2; i++) {
            ...
            tBH2_J2 += sigma[j]
                    * fFunction(sl, L[j], m_DMSW_lplus, s, m_DMSW_kPrime,
                            m_DMSW_k)
                    * gFunction(sl, m_DMSW_lminus, rPrime[i], L[j]);
            tBH2_J2 += sigma[j]
                    * fFunction(sl, L[j], m_DMSW_lplus, s, m_DMSW_kPrime,
                            m_DMSW_k)
                    * gFunction(sl, m_DMSW_lminus, r[i], L[j]);
...
std::complex<double> DDVCSProcessDMSW22::ampliVCS(int s2, int s1, int sl,
    ...
    tVCS_T1 = fFunction(sl, m_DMSW_lminus_tMin, m_DMSW_lplus_tMin, s,
            ...
            - (s0n_tMin * s0star_tMin / m_DMSW_2pBarqBar_tMin)
    ...
    tVCS_T1 *= (s0n_tMin / m_DMSW_2pBarqBar_tMin) * (m_cffH + m_cffE)
...
double DDVCSProcessDMSW22::crossSectionBH(int targetPolariz) {
    ...
    } else if (targetPolariz == +1 || targetPolariz == -1) {
        s = m_beamHelicity;

        for (s2 = -1; s2 < 2; s2 = s2 + 2) {
            for (sl = -1; sl < 2; sl = sl + 2) {

                Abh1 = Flong(targetPolariz, +1) * ampliBH1(s2, +1, sl, s)
                        + Flong(targetPolariz, -1) * ampliBH1(s2, -1, sl, s);

                Abh1crossed = Flong(targetPolariz, +1)
                ...
                // BH1-BH2 interference
                T2_bh12 += (Abh1 + Abh1crossed) * conj(Abh2 + Abh2crossed)
                        + conj((Abh1 + Abh1crossed) * conj(Abh2 + Abh2crossed));
...
double DDVCSProcessDMSW22::crossSectionVCS(int targetPolariz) {
    ...
    if (targetPolariz == 0) {
        ...
        XSEC =
                (1. / m_DMSW_jac) * pow(Constant::FINE_STRUCTURE_CONSTANT, 4.)
                        * m_xB * pow(m_DMSW_y, 2.) * real(T2_vcs)
                        * (1. / pow(m_DMSW_charge_e, 8.)) * sin(m_DMSW_thetaL)
                        / (16 * pow(2. * M_PI, 3.) * pow(m_Q2, 2.)
                                * sqrt(
                                        1.
        ...
        for (s2 = -1; s2 < 2; s2 = s2 + 2) {
            for (sl = -1; sl < 2; sl = sl + 2) {
                ...
                Avcs = Flong(targetPolariz, +1) * ampliVCS(s2, +1, sl, s)
                        + Flong(targetPolariz, -1) * ampliVCS(s2, -1, sl, s);
        ...
        //TODO TRANSVERSELY target polarization case
        return 0.;
    }
...
double DDVCSProcessDMSW22::crossSectionInterf(int targetPolariz) {
    ...
    std::complex<double> T2_interf2 = (0., 0.);
    ...
    if (targetPolariz == 0) {
        ...
        XSEC =
                (1. / m_DMSW_jac) * pow(Constant::FINE_STRUCTURE_CONSTANT, 4.)
                        * m_xB * pow(m_DMSW_y, 2.) * real(T2_interf)
                        * (1. / pow(m_DMSW_charge_e, 8.)) * sin(m_DMSW_thetaL)
                        / (16 * pow(2. * M_PI, 3.) * pow(m_Q2, 2.)
                                * sqrt(
                                        1.
...
std::complex<double> DDVCSProcessDMSW22::Yfunction(int s2, int s1) const {
    ...
    //For original t since r1, r2, r'1, r'2 build up p and p' which are soft part momenta
    if (s2 == +1 && s1 == +1) {
...
std::complex<double> DDVCSProcessDMSW22::J15plus(int s2, int s1) const {
    ...
    return s0n * J15value / m_DMSW_2pBarqBar_tMin;
}
...
//F matrix for longitudinally polarized target (collinear or counter-collinear to the incoming electron beam)
std::complex<double> DDVCSProcessDMSW22::Flong(int h1, int s1) const {
...
void DDVCSProcessDMSW22::computeInternalVariables(double Mnucleon) {
...
//We change from Trento's phi (m_phi) to TRF-II's phi (m_DMSW_phi)
//Trento convention: DOI 10.1103/PhysRevD.70.117504
//if m_phi = pi + alpha (with alpha > 0), then m_DMSW_phi = 2pi - alpha = 2pi - (m_phi - pi) = 3pi - m_phi
    if (m_phi >= 0. && m_phi <= M_PI) {
        m_DMSW_phi = M_PI - m_phi;
    } else {
        m_DMSW_phi = 3. * M_PI - m_phi;
    }
...
//Evaluation of tMin:
    ...
    m_DMSW_tMin = -1. / (4. * m_xB * (1. - m_xB) + m_DMSW_epsilon2);
    m_DMSW_tMin *= (2. * ((1. - m_xB) * m_Q2 - m_xB * m_Q2Prim)
            + m_DMSW_epsilon2 * (m_Q2 - m_Q2Prim)
            - 2. * sqrt(1. + m_DMSW_epsilon2)
                    * sqrt(
                            pow((1. - m_xB) * m_Q2 - m_xB * m_Q2Prim, 2.)
                                    - m_DMSW_epsilon2 * m_Q2 * m_Q2Prim));

//Basic numbers:
    m_DMSW_Mnucleon = Mnucleon;
    m_DMSW_charge_e = sqrt(Constant::FINE_STRUCTURE_CONSTANT * 4. * M_PI);
    m_DMSW_Q2Bar = 0.5 * (m_Q2 - m_Q2Prim + m_t / 2.);

//Light-like vectors (r1, r2, r'1, r'2) to build proton momenta (p, pPrim):
...
//Because of eq 19 at t = tMin (m_DMSW_sinGamma = 0), qPrim = (w2, 0, 0, w2*modv) and momentum conservation:
    if (m_t == m_DMSW_tMin) {
        sinThetaN = 0.;
    }

    double w2 = sqrt(m_Q2 / m_DMSW_epsilon2) + m_t / (2. * Mnucleon); //eq 17 in BM2003
    double modv = sqrt(1. - m_Q2Prim / pow(w2, 2.)); //modulus of outgoing photon's speed in TRF-I frame, eq 17 in BM2003

    m_DMSW_cosGamma = -(sqrt(m_DMSW_epsilon2) * 0.5 * (m_Q2 - m_Q2Prim + m_t)
            + sqrt(m_Q2) * w2)
            / (sqrt(m_Q2 * (1. + m_DMSW_epsilon2)) * modv * w2); //cosine of angle between outgoing photon and z-axis in TRF-I frame, eq 18 in BM2003
    m_DMSW_sinGamma = sqrt(1. - pow(m_DMSW_cosGamma, 2.)); //this expression can produce NaNs when t = tMin (probably because numerics since for t = tMin, m_DMSW_cosGamma == -1 so m_DMSW_sinGamma == 0)

//Because of eq 21 in BM2003, to avoid numerical effects:
    if (m_t == m_DMSW_tMin) {
        m_DMSW_cosGamma = 1.; //TODO PS added, check if makes sense
        m_DMSW_sinGamma = 0.;
    }

    double pPrimI[4]; //pPrim in TRF-I frame, eqs 12 and 13 in BM2003

    pPrimI[0] = Mnucleon - m_t / (2. * Mnucleon);
    pPrimI[1] = modpPrimI * sinThetaN * cos(m_DMSW_phi);
    pPrimI[2] = modpPrimI * sinThetaN * sin(m_DMSW_phi);
    pPrimI[3] = modpPrimI * cosThetaN;

    m_DMSW_pPrim[0] = pPrimI[0];
    m_DMSW_pPrim[1] = m_DMSW_cosGamma
            * (cos(M_PI + m_DMSW_phi) * pPrimI[1]
                    + sin(M_PI + m_DMSW_phi) * pPrimI[2])
            - m_DMSW_sinGamma * pPrimI[3];
    m_DMSW_pPrim[2] = -sin(M_PI + m_DMSW_phi) * pPrimI[1]
            + cos(M_PI + m_DMSW_phi) * pPrimI[2];
    m_DMSW_pPrim[3] = m_DMSW_sinGamma
            * (cos(M_PI + m_DMSW_phi) * pPrimI[1]
                    + sin(M_PI + m_DMSW_phi) * pPrimI[2])
            + m_DMSW_cosGamma * pPrimI[3];
    ...
    for (i = 0; i < 4; i++) {
        m_DMSW_pBar[i] = 0.5 * (m_DMSW_p[i] + m_DMSW_pPrim[i]);
    }
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (i == 1 || i == 2) {
            m_DMSW_rPrime2[i] += m_DMSW_pPrim[i];
        }
    }
...
//Other momenta:
    ...
    m_DMSW_cosTheta_e = -(1. + m_DMSW_y * m_DMSW_epsilon2 / 2.)
            / sqrt(1. + m_DMSW_epsilon2);
    m_DMSW_sinTheta_e = sqrt(1. - pow(m_DMSW_cosTheta_e, 2.));

    m_DMSW_k[0] = m_E;
    m_DMSW_k[1] = m_E
            * (m_DMSW_sinTheta_e * m_DMSW_cosGamma * cos(M_PI + m_DMSW_phi)
                    - m_DMSW_cosTheta_e * m_DMSW_sinGamma);
    m_DMSW_k[2] = -m_E * m_DMSW_sinTheta_e * sin(M_PI + m_DMSW_phi);
    m_DMSW_k[3] = m_E
            * (m_DMSW_sinTheta_e * m_DMSW_sinGamma * cos(M_PI + m_DMSW_phi)
                    + m_DMSW_cosTheta_e * m_DMSW_cosGamma);

    for (i = 0; i < 4; i++) {
        m_DMSW_Delta[i] = m_DMSW_pPrim[i] - m_DMSW_p[i];
    }

    m_DMSW_qPrim[0] = w2;
    m_DMSW_qPrim[1] = 0.;
    m_DMSW_qPrim[2] = 0.;
    m_DMSW_qPrim[3] = w2 * modv;

    double w1 = sqrt(m_Q2 / m_DMSW_epsilon2);

    m_DMSW_q[0] = w1;
    m_DMSW_q[1] = w1 * sqrt(1. + m_DMSW_epsilon2) * m_DMSW_sinGamma;
    m_DMSW_q[2] = 0.;
    m_DMSW_q[3] = -w1 * sqrt(1. + m_DMSW_epsilon2) * m_DMSW_cosGamma;

    m_DMSW_lminus[0] = 0.5 * w2 * (1. + modv * cos(m_DMSW_thetaL));
    m_DMSW_lminus[1] = 0.5 * sqrt(m_Q2Prim) * sin(m_DMSW_thetaL)
            * cos(m_DMSW_phiL);
    m_DMSW_lminus[2] = 0.5 * sqrt(m_Q2Prim) * sin(m_DMSW_thetaL)
            * sin(m_DMSW_phiL);
    m_DMSW_lminus[3] = 0.5 * w2 * (modv + cos(m_DMSW_thetaL));

    for (i = 0; i < 4; i++) {
        m_DMSW_lplus[i] = m_DMSW_qPrim[i] - m_DMSW_lminus[i];
        m_DMSW_kPrime[i] = m_DMSW_k[i] - m_DMSW_q[i];
    }

// rho variable and 2pBarqBar scale:
    m_DMSW_rho = 2. * m_DMSW_Q2Bar / (2. * m_Q2 / m_xB - m_Q2 - m_Q2Prim + m_t); //xi variable, the first of the two eqs in BM2003's eq 31 (REMEMBER: we take positive eta)
    m_DMSW_2pBarqBar = m_DMSW_Q2Bar / m_DMSW_rho;

// n and n^\star light-like vectors from BM2000 and BM2003. Here, n = m_DMSW_nBM and n^\star = m_DMSW_nstarBM:
    double delta2 = (Mnucleon * Mnucleon - m_t / 4.); //delta2 = \delta^2 * m_DMSW_Q2Bar with \delta^2 as defined in BM2003 paper (eq A1)
    double root = sqrt(1. + 4. * m_DMSW_rho * delta2 / m_DMSW_2pBarqBar);

    for (i = 0; i < 4; i++) {
        m_DMSW_nstarBM[i] = -(delta2 / m_DMSW_2pBarqBar) * (1. / root)
                * (m_DMSW_q[i] + m_DMSW_qPrim[i]) / 2.
                + ((1. + root) / (4. * root)) * 2. * m_DMSW_pBar[i];
        m_DMSW_nBM[i] = (2. / (m_DMSW_2pBarqBar * root))
                * (m_DMSW_q[i] + m_DMSW_qPrim[i]) / 2.
                - ((1. - root) / (2. * delta2 * root)) * 2. * m_DMSW_pBar[i];
        m_DMSW_HATnBM[i] = m_DMSW_2pBarqBar * m_DMSW_nBM[i];
    }

// Proceeding to compute vectors and variables in hard part at t = tMin:
    double delta2_tMin = (Mnucleon * Mnucleon - m_DMSW_tMin / 4.);
    m_DMSW_Q2Bar_tMin = 0.5 * (m_Q2 - m_Q2Prim + m_DMSW_tMin / 2.);
    m_DMSW_rho_tMin = 2. * m_DMSW_Q2Bar_tMin
            / (2. * m_Q2 / m_xB - m_Q2 - m_Q2Prim + m_DMSW_tMin); //xi variable, the first of the two eqs in BM2003's eq 31 (REMEMBER: we take positive eta)
    m_DMSW_2pBarqBar_tMin = m_DMSW_Q2Bar_tMin / m_DMSW_rho_tMin;
    double root_tMin = sqrt(
            1. + 4. * m_DMSW_rho_tMin * delta2_tMin / m_DMSW_2pBarqBar_tMin);
    double w2_tMin = sqrt(m_Q2 / m_DMSW_epsilon2)
    ...
    double cosThetaN_tMin = -(m_DMSW_epsilon2 * 0.5
            * (m_Q2 + m_Q2Prim - m_DMSW_tMin) - m_xB * m_DMSW_tMin)
            / (2 * m_xB * Mnucleon * sqrt(1. + m_DMSW_epsilon2) * modp2I_tMin);
    double sinThetaN_tMin = 0.;

    m_DMSW_qPrim_tMin[0] = w2_tMin;
    m_DMSW_qPrim_tMin[1] = 0.;
    m_DMSW_qPrim_tMin[2] = 0.;
    m_DMSW_qPrim_tMin[3] = w2_tMin * modv_tMin;

    m_DMSW_q_tMin[0] = w1;
    m_DMSW_q_tMin[1] = w1 * sqrt(1. + m_DMSW_epsilon2) * m_DMSW_sinGamma_tMin;
    m_DMSW_q_tMin[2] = 0.;
    m_DMSW_q_tMin[3] = -w1 * sqrt(1. + m_DMSW_epsilon2) * m_DMSW_cosGamma_tMin;
    ...
    m_DMSW_pPrim_tMin[1] = m_DMSW_cosGamma_tMin
            * (cos(M_PI + m_DMSW_phi) * p2I_tMin[1]
                    + sin(M_PI + m_DMSW_phi) * p2I_tMin[2])
            - m_DMSW_sinGamma_tMin * p2I_tMin[3];
    m_DMSW_pPrim_tMin[2] = -sin(M_PI + m_DMSW_phi) * p2I_tMin[1]
            + cos(M_PI + m_DMSW_phi) * p2I_tMin[2];
    m_DMSW_pPrim_tMin[3] = m_DMSW_sinGamma_tMin
            * (cos(M_PI + m_DMSW_phi) * p2I_tMin[1]
                    + sin(M_PI + m_DMSW_phi) * p2I_tMin[2])
            + m_DMSW_cosGamma_tMin * p2I_tMin[3];
    ...
    for (i = 0; i < 4; i++) {
        m_DMSW_nstarBM_tMin[i] = -(delta2_tMin / m_DMSW_2pBarqBar_tMin)
                * (1. / root_tMin) * (m_DMSW_q_tMin[i] + m_DMSW_qPrim_tMin[i])
                / 2.
                + ((1. + root_tMin) / (4. * root_tMin)) * 2.
                        * m_DMSW_pBar_tMin[i];
        m_DMSW_nBM_tMin[i] = (2. / (m_DMSW_2pBarqBar_tMin * root_tMin))
                * (m_DMSW_q_tMin[i] + m_DMSW_qPrim_tMin[i]) / 2.
                - ((1. - root_tMin) / (2. * delta2_tMin * root_tMin)) * 2.
                        * m_DMSW_pBar_tMin[i];
        m_DMSW_HATnBM_tMin[i] = m_DMSW_2pBarqBar_tMin * m_DMSW_nBM_tMin[i];
    }
...
//Evaluation of r'1 and r'2 at t = tMin:
    ...
    double Alpha_tMin = MinkProd(m_DMSW_nminus_tMin, m_DMSW_pPrim_tMin);
    double Beta_tMin = MinkProd(m_DMSW_nplus_tMin, m_DMSW_pPrim_tMin);
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (i == 1 || i == 2) {
            m_DMSW_rPrime2_tMin[i] += m_DMSW_pPrim_tMin[i];
        }
    }

    m_DMSW_k_tMin[0] = m_E;
    m_DMSW_k_tMin[1] = m_E
            * (m_DMSW_sinTheta_e * m_DMSW_cosGamma_tMin * cos(M_PI + m_DMSW_phi)
                    - m_DMSW_cosTheta_e * m_DMSW_sinGamma_tMin);
    m_DMSW_k_tMin[2] = -m_E * m_DMSW_sinTheta_e * sin(M_PI + m_DMSW_phi);
    m_DMSW_k_tMin[3] = m_E
            * (m_DMSW_sinTheta_e * m_DMSW_sinGamma_tMin * cos(M_PI + m_DMSW_phi)
                    + m_DMSW_cosTheta_e * m_DMSW_cosGamma_tMin);

    m_DMSW_lminus_tMin[0] = 0.5 * w2_tMin
            * (1. + modv_tMin * cos(m_DMSW_thetaL));
    m_DMSW_lminus_tMin[1] = 0.5 * sqrt(m_Q2Prim) * sin(m_DMSW_thetaL)
            * cos(m_DMSW_phiL);
    m_DMSW_lminus_tMin[2] = 0.5 * sqrt(m_Q2Prim) * sin(m_DMSW_thetaL)
            * sin(m_DMSW_phiL);
    m_DMSW_lminus_tMin[3] = 0.5 * w2_tMin * (modv_tMin + cos(m_DMSW_thetaL));

    for (i = 0; i < 4; i++) {
        m_DMSW_lplus_tMin[i] = m_DMSW_qPrim_tMin[i] - m_DMSW_lminus_tMin[i];
        m_DMSW_kPrime_tMin[i] = m_DMSW_k_tMin[i] - m_DMSW_q_tMin[i];
    }

//EM form factors for proton:
    m_DMSW_F1 = (4. * pow(Mnucleon, 2.) - m_t * 2.7928)
            * pow((1. - m_t / 0.71), -2.) / (4. * pow(Mnucleon, 2.) - m_t); //2.7928 = Born's magneton for proton

    m_DMSW_F2 = 4. * pow(Mnucleon, 2.) * (2.7928 - 1.)
            * pow((1. - m_t / 0.71), -2.) / (4. * pow(Mnucleon, 2.) - m_t);
