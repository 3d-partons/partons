...
double GPDEvolutionVinnikov::conv_int_ns(int const ix, double const xi,
    ...
    if (ix < 3 * n) {
        ...
        if (ix > (2 * n + 2)) {
            if (0 == (ix % 2))
                ...
                in = ix - 1;
            for (i = 2 * n + 2; i < in; i += 2) {
                double y = yy[i];
                double f1 = (shift + y) * (gpd[i] - gpd_x) * (x - xi)
                        * (y - xp2xi) / (xi * (y - xi) * (y - x));
                convint += 2.0 * f1;
            }
            for (i = 2 * n + 1; i < in; i += 2) {
                double y = yy[i];
                double f1 = (shift + y) * (gpd[i] - gpd_x) * (x - xi)
                        * (y - xp2xi) / (xi * (y - xi) * (y - x));
                convint += 4.0 * f1;
...
double GPDEvolutionVinnikov::conv_int_qq(int const ix, double const xi,
        int const n, const std::vector<double>& gpd,
        const std::vector<double>& yy) {
    /*
...
double GPDEvolutionVinnikov::conv_int_gg(int const ix, double const xi,
        int const n, const std::vector<double>& gpd,
        const std::vector<double>& yy) {
    /*
     Dimension of "gpd" and "yy" arrays must be 2n+1, i.e. "double gpd[2n+1]"
    ...
    if (ix < n) {
        ...
        convint += gpd[ix] * (-0.5 * C_A - 2.0 * c_1_3 * T_R);
    ...
    if (ix == n) {
        ...
        convint += (-0.5 * C_A - 2.0 * c_1_3 * T_R) * gpd[ix];
...
double GPDEvolutionVinnikov::conv_int_gq(int const ix, double const xi,
        int const n, const std::vector<double>& gpd,
        const std::vector<double>& yy) {
    /*
     Dimension of "gpd" and "yy" arrays must be 2n+1, i.e. "double gpd[2n+1]"
    ...
    if ((2 * n - 2) == ix) {
        ...
        double f1 = gpd[2 * n - 1] * (xi2 - x2 - 2.0 * y2 + 2.0 * x * y)
                / (xi2 - y2);
        double f2 = gpd[2 * n - 2];
        return C_F * c_1_3 * gamma * (4.0 * f1 * (shift + y) + f2 * (shift + x));
    }
...
double GPDEvolutionVinnikov::conv_int_qg(int const ix, double const xi,
        int const n, const std::vector<double>& gpd,
        const std::vector<double>& yy) {
    /*
     Dimension of "gpd" and "yy" arrays must be 2n+1, i.e. "double gpd[2n+1]"
    ...
    if (n == ix) {
        ...
        }
        {
            ...
            if ((2 * n - 1) == in) {
                double ya = yy[2 * n - 2];
                double f1a = (shift + ya) * (gpd[i] - gpd_n)
                        / ((xi + ya) * (xi + ya));
...
double GPDEvolutionVinnikov::conv_int_qq_pol(int const ix, double const xi,
        int const n, const std::vector<double>& gpd,
        const std::vector<double>& yy) {
    /*
     Dimension of "gpd" and "yy" arrays must be 2n+1, i.e. "double gpd[2n+1]"
    ...
    if ((2 * n - 1) == ix) {
        ...
        double y2 = y * y;
        double f1 = (shift + y) * (gpd[2 * n - 2] - gpd[ix])
                * (2.0 * xi2 - x2 - y2) / ((y - x) * (xi2 - y2));
        double f2 = (gpd[2 * n - 3] - 6.0 * gpd[2 * n - 2]
                + 3.0 * gpd[2 * n - 1]) / (3.0 * gamma);
        double f3 = -(shift + 1.0) * gpd[ix] * (2.0 * xi2 - x2 - 1.0)
                / ((1.0 - x) * (xi2 - 1.0));
        return C_F
                * (c_1_3 * gamma * (-0.25 * f1 + 2.0 * f2 + 1.25 * f3)
                        + gpd[ix]
    ...
    if ((2 * n - 2) == ix) {
        ...
        double f2 = (shift + y) * (gpd[2 * n - 1] - gpd[ix])
                * (2.0 * xi2 - x2 - y2) / ((y - x) * (xi2 - y2));
        double f3 = -(shift + 1.0) * gpd[ix] * (xi2 - x2 - 1.0)
                / ((1.0 - x) * (xi2 - 1.0));
        return C_F
                * (c_1_3 * gamma * (f1 + 4.0 * f2 + f3)
                        + gpd[ix]
    ...
    if (ix < n) {
        ...
        if (1 == ix) {
            ...
            double f0 = shift * (gpd[0] - gpd_x) * twoxixpx2 / (-xi2 * x2);

            double f1 = (-2.0 * gpd[0] - 3.0 * gpd[1] + 6.0 * gpd[2] - gpd[3])
                    / (6.0 * gamma * (xi - x));

            double f2 = (shift + y) * (gpd[2] - gpd_x) * (twoxixpx2 + y2)
                    / ((xi2 - y2) * (y2 - x2));
        ...
        if (2 == ix) {
            ...
            double f1 = (shift + y) * (gpd[1] - gpd_x) * (twoxixpx2 + y2)
                    / ((xi2 - y2) * (y2 - x2));

            double f2 = (-2.0 * gpd[1] - 3.0 * gpd[2] + 6.0 * gpd[3] - gpd[4])
                    / (6.0 * gamma * (xi - x));
        ...
        if (ix > 2) {
            ...
            for (i = 2; i < in; i += 2) {
                double y = yy[i];
                double y2 = y * y;
                double f1 = (shift + y) * (gpd[i] - gpd_x) * (twoxixpx2 + y2)
                        / ((xi2 - y2) * (y2 - x2));
                convint += 2.0 * f1;
            }
            for (i = 1; i < in; i += 2) {
            ...
            if ((ix - 1) == in) {
                ...
                double yb = yy[ix - 1];
                double yb2 = yb * yb;
                double f1 = (shift + ya) * (gpd[ix - 2] - gpd_x)
                        * (twoxixpx2 + ya2) / ((xi2 - ya2) * (ya2 - x2));
                double f2 = (shift + yb) * (gpd[ix - 1] - gpd_x)
                        * (twoxixpx2 + yb2) / ((xi2 - yb2) * (yb2 - x2));
                double f3 = (-2.0 * gpd[ix - 3] + 9.0 * gpd[ix - 2]
                        - 18.0 * gpd[ix - 1] + 11.0 * gpd[ix])
                        / (6.0 * gamma * (xi - x));
            ...
            {
                // y = 0
                convint += shift * (gpd[0] - gpd_x) * twoxixpx2 / (-xi2 * x2);
            }
        ...
        }
        {
            ...
            if ((2 * n - 1) == in) {
                ...
                double f1a = (shift + ya) * (gpd[2 * n - 2] - gpd_x)
                        * (2.0 * xi * ya + x2 + ya * ya)
                        / ((ya * ya - x2) * (xi + ya));
                double f1b = -(shift + 1.0) * gpd_x * (2.0 * xi + x2 + 1.0)
                        / ((1.0 - x2) * (xi + 1.0));
                convint += 2.0 * f1 - 0.25 * f1a + 1.25 * f1b;
            }
        }
    ...
    if (ix < (2 * n - 2)) {
        double convint = 0.0;
        double const gpd_x = gpd[ix];
        double const twoxi2mx2 = 2.0 * xi2 - x2;
        int i, in;
        if (0 == (ix % 2))
            in = 2 * n;
        else
        ...
        for (i = ix + 2; i < in; i += 2) {
            double y = yy[i];
            double f1 = (shift + y) * (gpd[i] - gpd_x) * (twoxi2mx2 - y * y)
                    / ((y - x) * (xi2 - y * y));
            convint += 2.0 * f1;
        }
        for (i = ix + 1; i < in; i += 2) {
        ...
        {
            double y = yy[in];
            double f1 = (shift + y) * (gpd[in] - gpd_x) * (twoxi2mx2 - y * y)
                    / ((y - x) * (xi2 - y * y));
            convint += f1;
            if ((2 * n - 1) == in) {
                double ya = yy[2 * n - 2];
                double f1a = (shift + ya) * (gpd[2 * n - 2] - gpd_x)
                        * (twoxi2mx2 - ya * ya) / ((ya - x) * (xi2 - ya * ya));
                double f1b = -(shift + 1.0) * gpd_x * (twoxi2mx2 - 1.0)
                        / ((1.0 - x) * (xi2 - 1.0));
                convint += 2.0 * f1 - 0.25 * f1a + 1.25 * f1b;
            }
        }
...
double GPDEvolutionVinnikov::conv_int_gg_pol(int const ix, double const xi,
        int const n, const std::vector<double>& gpd,
        const std::vector<double>& yy) {
    /*
     Dimension of "gpd" and "yy" arrays must be 2n+1, i.e. "double gpd[2n+1]"
    ...
    if ((2 * n - 1) == ix) {
        ...
        double y2 = y * y;
        double f1 =
                (shift + y) / ((xi2 - y2) * (xi2 - y2) * (y - x))
                        * (4.0 * gpd[2 * n - 2]
                                * (x * (xi2 + y2) - y * (xi2 + x2)) * (y - x)
                                + 2.0 * (gpd[2 * n - 2] - gpd[ix])
                                        * ((xi2 + x2) * (xi2 + y2)
                                                - 4.0 * xi2 * x * y));
        double f2 = (gpd[2 * n - 3] - 6.0 * gpd[2 * n - 2]
                + 3.0 * gpd[2 * n - 1]) / (3.0 * gamma);
        double f3 = -(shift + 1.0) / ((xi2 - 1.0) * (xi2 - 1.0) * (1.0 - x))
                * 2.0 * gpd[ix] * ((xi2 + x2) * (xi2 + 1.0) - 4.0 * xi2 * x);

        return C_A
                * (c_1_3 * gamma * (-0.25 * f1 + 2.0 * f2 + 1.25 * f3)
                        + gpd[ix]
                                * (log((1.0 - x) * (1.0 - x) / (1.0 - xi2))
                                        + (2.0 * x - 2.0) / (1.0 - xi2)))
                + gpd[ix] * (11.0 / 6.0 * C_A - 2.0 / 3.0 * T_R);
    }
    ...
    if (ix < n) {
        double convint = 0.0;
        double gpd_x = gpd[ix];
        double xix1 = xi2 + 2.0 * x2 + 2.0 * xi * x;
        double ximx2 = (xi - x) * (xi - x);
        int i, in, i0;
        ...
        if (1 == ix) {
            ...
            double f1 = -2.0 * (shift + x) * x * (gpd[1] - gpd[n]) / (xi2 - x2)
                    - (-3.0 * gpd[1] + 6.0 * gpd[2] - gpd[3]) / (12.0 * gamma);
            double f2 = (shift + y) * (y2 - xix1) * y
                    * ((xi - x) * (gpd[2] - gpd[n])
                            / ((xi2 - y2) * (xi2 - y2) * (xi + x))
                            + (gpd[2] - gpd[n])
                                    / ((xi2 - y2) * (xi + x) * (xi + x))
        ...
        if (2 == ix) {
            ...
            double f1 = (shift + y) * (y2 - xix1) * y
                    * ((xi - x) * (gpd[1] - gpd[n])
                            ...
                            + (gpd[1] - gpd_x)
                                    / ((y2 - x2) * (xi + x) * (xi + x)));
            double f2 = -2.0 * (shift + x) * x * (gpd[2] - gpd[n]) / (xi2 - x2)
                    - (-6.0 * gpd[1] + 3.0 * gpd[2] + 2.0 * gpd[3])
                            / (12.0 * gamma);
            convint += 4.0 * f1 + f2;
        }
        ...
        if (ix > 2) {
            ...
            {
                // y = x
                convint += -2.0 * (shift + x) * x * (gpd[ix] - gpd[n])
                        / (xi2 - x2)
                        - (-2.0 * gpd[ix - 3] + 9.0 * gpd[ix - 2]
                                - 18.0 * gpd[ix - 1] + 11.0 * gpd[ix])
                                / (12.0 * gamma);
            ...
            if (1 == i0) {
                ...
                double yb = yy[2];
                double yb2 = yb * yb;
                double f1 = (shift + ya) * (ya2 - xix1) * ya
                        * ((xi - x) * (gpd[1] - gpd[n])
                                / ((xi2 - ya2) * (xi2 - ya2) * (xi + x))
                                + (gpd[1] - gpd[n])
                                        / ((xi2 - ya2) * (xi + x) * (xi + x))
                                + (gpd[1] - gpd_x)
                                        / ((ya2 - x2) * (xi + x) * (xi + x)));
                double f2 = (shift + yb) * (yb2 - xix1) * yb
                        * ((xi - x) * (gpd[2] - gpd[n])
                                / ((xi2 - yb2) * (xi2 - yb2) * (xi + x))
                                + (gpd[2] - gpd[n])
                                        / ((xi2 - yb2) * (xi + x) * (xi + x))
                                + (gpd[2] - gpd_x)
                                        / ((yb2 - x2) * (xi + x) * (xi + x)));
                convint += 3.0 * f1 - 0.25 * f2;
            }
        ...
        for (i = ix + 2; i < in; i += 2) {
            double y = yy[i];
            double f1 = (shift + y) * (gpd[i] - gpd_x) * x
                    * (xi2 + 2.0 * xi * y + 2.0 * y * y - x2)
                    / ((xi + y) * (xi + y) * (y * y - x2));
            convint += 2.0 * f1;
        }
        for (i = ix + 1; i < in; i += 2) {
        ...
        {
            double y = yy[in];
            double f1 = (shift + y) * (gpd[in] - gpd_x) * x
                    * (xi2 + 2.0 * xi * y + 2.0 * y * y - x2)
                    / ((xi + y) * (xi + y) * (y * y - x2));
            convint += f1;
            if ((2 * n - 1) == in) {
                double ya = yy[2 * n - 2];
                double f1a = (shift + ya) * (gpd[2 * n - 2] - gpd_x) * x
                        * (xi2 + 2.0 * xi * ya + 2.0 * ya * ya - x2)
                        / ((xi + ya) * (xi + ya) * (ya * ya - x2));
                double f1b = -(shift + 1.0) * gpd_x * x
                        * (xi2 + 2.0 * xi + 2.0 - x2)
                        / ((xi + 1.0) * (xi + 1.0) * (1.0 - x2));
                convint += 2.0 * f1 - 0.25 * f1a + 1.25 * f1b;
            }
        }
        {
        ...
        convint += gpd[ix]
                * (2.0 * x * (1.0 - x) / ((xi + x) * (1.0 + x))
                        - 2.0 * x2 * x / (xi2 * (xi + x))
                        + log(x2 * (1.0 - x) / (xi2 * (1.0 + x))));

        convint += -(gpd[n] - gpd[ix])
                * (log(xi2 / (xi2 - x2)) + 2.0 * x2 * x / (xi2 * (xi + x))
                        + x2 / ((xi + x) * (xi + x)));

        convint *= C_A;

        convint += gpd[ix] * (11.0 / 6.0 * C_A - 2.0 / 3.0 * T_R);
    ...
    if (ix == n) {
        ...
        for (i = i0 + 2; i < 2 * n; i += 2) {
            double y = yy[i];
            double f1 = -4.0 * xi * y * (shift + y) / ((xi2 - y * y) * (xi + y))
                    * (gpd[i] - gpd[n]);
            convint += 2.0 * f1;
        }
        for (i = i0 + 1; i < 2 * n; i += 2) {
        ...
        {
            // y = 1
            convint += 4.0 * xi * (shift + 1.0) / ((xi2 - 1.0) * (xi + 1.0))
                    * gpd[n];
        }
        if (ix == i0) {
            convint += (-11.0 * gpd[ix] + 18.0 * gpd[ix + 1] - 9.0 * gpd[ix + 2]
        ...
        if ((ix + 1) == i0) {
            double ya = yy[ix + 1];
            double yb = yy[ix + 2];
            double f0 = (-11.0 * gpd[ix] + 18.0 * gpd[ix + 1]
                    - 9.0 * gpd[ix + 2] + 2.0 * gpd[ix + 3]) / (6.0 * gamma);
            double f1 = -4.0 * xi * ya * (shift + ya)
                    / ((xi2 - ya * ya) * (xi + ya)) * (gpd[ix + 1] - gpd[n]);
            double f2 = -4.0 * xi * yb * (shift + yb)
                    / ((xi2 - yb * yb) * (xi + yb)) * (gpd[ix + 2] - gpd[n]);
            convint += 1.25 * f0 + 3.0 * f1 - 0.25 * f2;
        }

        convint *= gamma * c_1_3;

        convint += gpd[ix] * (log((1.0 - xi) / (1.0 + xi)) - 2.0 / (1.0 + xi));

        convint += gpd[n] * (-4.0 * xi / (xi + 1.0) + 2.0);
    ...
    if (ix < (2 * n - 2)) {
        ...
        for (i = i0 + 2; i < 2 * n; i += 2) {
            double y = yy[i];
            double y2 = y * y;
            double f1 = (shift + y) / ((xi2 - y2) * (xi2 - y2)) * 4.0
                    * (gpd[i] - gpd[n]) * (x * (xi2 + y2) - y * (xi2 + x2))
                    + (shift + y) / ((xi2 - y2) * (xi2 - y2)) * 2.0
                            * (gpd[i] - gpd[n])
                            * (3.0 * xi2 * y - xi2 * x - y2 * x - y2 * y)
                    + 2.0 * (shift + y) * (gpd[i] - gpd_x) / (y - x);
            convint += 2.0 * f1;
        }
        for (i = i0 + 1; i < 2 * n; i += 2) {
            double y = yy[i];
        ...
        {
            // y = 1
            convint += (shift + 1.0) / ((xi2 - 1.0) * (xi2 - 1.0)) * 4.0
                    * (-gpd[n]) * (x * (xi2 + 1.0) - (xi2 + x2))
                    + (shift + 1.0) / ((xi2 - 1.0) * (xi2 - 1.0)) * 2.0
                            * (-gpd[n]) * (3.0 * xi2 - xi2 * x - x - 1.0)
                    + 2.0 * (shift + 1.0) * (-gpd_x) / (1.0 - x);
        }
        if (ix == i0) {
            convint += (shift + x) / (xi2 - x2) * 2.0 * (gpd[ix] - gpd[n]) * 2.0
                    * x
                    + (-11.0 * gpd[ix] + 18.0 * gpd[ix + 1] - 9.0 * gpd[ix + 2]
                            + 2.0 * gpd[ix + 3]) / (3.0 * gamma);
        }
        if ((ix + 1) == i0) {
            ...
            double ya2 = ya * ya;
            double yb2 = yb * yb;
            double f0 = (shift + x) / (xi2 - x2) * 2.0 * (gpd[ix] - gpd[n])
                    * 2.0 * x
                    + (-11.0 * gpd[ix] + 18.0 * gpd[ix + 1] - 9.0 * gpd[ix + 2]
                            + 2.0 * gpd[ix + 3]) / (3.0 * gamma);
            double f1 = (shift + ya) / ((xi2 - ya2) * (xi2 - ya2)) * 4.0
                    * (gpd[ix + 1] - gpd[n])
                    * (x * (xi2 + ya2) - ya * (xi2 + x2))
                    + (shift + ya) / ((xi2 - ya2) * (xi2 - ya2)) * 2.0
                            * (gpd[ix + 1] - gpd[n])
                            * (3.0 * xi2 * ya - xi2 * x - ya2 * x - ya2 * ya)
                    + 2.0 * (shift + ya) * (gpd[ix + 1] - gpd_x) / (ya - x);
            double f2 = (shift + yb) / ((xi2 - yb2) * (xi2 - yb2)) * 4.0
                    * (gpd[ix + 2] - gpd[n])
                    * (x * (xi2 + yb2) - yb * (xi2 + x2))
                    + (shift + yb) / ((xi2 - yb2) * (xi2 - yb2)) * 2.0
                            * (gpd[ix + 2] - gpd[n])
                            * (3.0 * xi2 * yb - xi2 * x - yb2 * x - yb2 * yb)
                    + 2.0 * (shift + yb) * (gpd[ix + 2] - gpd_x) / (yb - x);
            convint += 1.25 * f0 + 3.0 * f1 - 0.25 * f2;
        }
        ...
        convint += gpd[n]
                * ((4.0 * x - 2.0 * xi2 - 2.0 * x2) / (xi2 - 1.0) + 2.0);

        convint += (gpd[n] - gpd[ix])
                * (2.0 * (xi2 - x) / (xi2 - 1.0) - 2.0
                        + log((xi2 - x2) / (xi2 - 1.0)));
...
double GPDEvolutionVinnikov::conv_int_gq_pol(int const ix, double const xi,
        int const n, const std::vector<double>& gpd,
        const std::vector<double>& yy) {
    /*
     Dimension of "gpd" and "yy" arrays must be 2n+1, i.e. "double gpd[2n+1]"
    ...
    if ((2 * n - 1) == ix) {
        double y = yy[2 * n - 2];
        double f1 = (shift + y) * gpd[2 * n - 2] * (xi2 + x2 - 2.0 * x * y)
                / (xi2 - y * y);
        double f2 = (shift + x) * gpd[ix];
        return C_F * c_1_3 * gamma * (2.0 * f2 - 0.25 * f1);
    }

    if ((2 * n - 2) == ix) {
        double y = yy[2 * n - 1];
        double f1 = (shift + x) * gpd[ix];
        double f2 = (shift + y) * gpd[2 * n - 1] * (xi2 + x2 - 2.0 * x * y)
                / (xi2 - y * y);
        return C_F * c_1_3 * gamma * (f1 + 4.0 * f2);
    }
    ...
    if (ix < n) {
        ...
        if (1 == ix) {
            double y = yy[2];
            double f1 = (shift + x) * gpd[1] * x / (xi2 - x2);
            double f2 = (shift + y) * gpd[2] * y / (xi2 - y * y);
            convint += 2.0 * f1 - 0.25 * f2;
        }

        if (2 == ix) {
            double y = yy[1];
            double f1 = (shift + y) * gpd[1] * y / (xi2 - y * y);
            double f2 = (shift + x) * gpd[2] * x / (xi2 - x2);
            convint += 4.0 * f1 + f2;
        }
        ...
        if (ix > 2) {
            ...
            for (i = 2; i < in; i += 2) {
                double y = yy[i];
                double f1 = (shift + y) * gpd[i] * y / (xi2 - y * y);
                convint += 2.0 * f1;
            }
            for (i = 1; i < in; i += 2) {
                double y = yy[i];
            ...
            {
                double y = yy[in];
                double f1 = (shift + y) * gpd[in] * y / (xi2 - y * y);
                convint += f1;
                if ((ix - 1) == in) {
                    double ya = yy[ix - 2];
                    double f1a = (shift + ya) * gpd[ix - 2] * ya
                            / (xi2 - ya * ya);
                    double f1b = (shift + x) * gpd[ix] * x / (xi2 - x2);
                    convint += 2.0 * f1 - 0.25 * f1a + 1.25 * f1b;
                }
            }
            {
        ...
        for (i = ix + 2; i < in; i += 2) {
            double y = yy[i];
            double f1 = (shift + y) * gpd[i] * 2.0 * x / (xi + y);
            convint += 2.0 * f1;
        }
        for (i = ix + 1; i < in; i += 2) {
            double y = yy[i];
        ...
        {
            double y = yy[in];
            double f1 = (shift + y) * gpd[in] * 2.0 * x / (xi + y);
            convint += f1;
            if ((2 * n - 1) == in) {
                double ya = yy[2 * n - 2];
                double f1a = (shift + ya) * gpd[2 * n - 2] * 2.0 * x
                        / (xi + ya);
                convint += 2.0 * f1 - 0.25 * f1a;
            }
        }
        ...
        {
            // y = x
            convint += (shift + x) * gpd[ix] * 2.0 * x / (xi + x);
        }
    ...
    if (n == ix) {
        ...
        if ((2 * n - 1) == in) {
            double y = yy[in];
            double ya = yy[2 * n - 2];
            double f1 = (shift + y) * gpd[in] / (xi + y);
            double f1a = (shift + ya) * gpd[2 * n - 2] / (xi + ya);
            convint += 2.0 * f1 - 0.25 * f1a;
        }
        convint *= 2.0 * xi;
    ...
    if (ix < (2 * n - 2)) {
        ...
        for (i = ix + 2; i < in; i += 2) {
            double y = yy[i];
            double f1 = (shift + y) * gpd[i] * (xi2 + x2 - 2.0 * x * y)
                    / (xi2 - y * y);
            convint += 2.0 * f1;
        }
        for (i = ix + 1; i < in; i += 2) {
        ...
        if ((2 * n - 1) == in) {
            double y = yy[in];
            double ya = yy[2 * n - 2];
            double f1 = (shift + y) * gpd[in] * (xi2 + x2 - 2.0 * x * y)
                    / (xi2 - y * y);
            double f1a = (shift + ya) * gpd[2 * n - 2]
                    * (xi2 + x2 - 2.0 * x * ya) / (xi2 - ya * ya);
            convint += 3.0 * f1 - 0.25 * f1a;
        }
        {
            // y = x
            convint += (shift + x) * gpd[ix];
...
double GPDEvolutionVinnikov::conv_int_qg_pol(int const ix, double const xi,
        int const n, const std::vector<double>& gpd,
        const std::vector<double>& yy) {
    /*
     Dimension of "gpd" and "yy" arrays must be 2n+1, i.e. "double gpd[2n+1]"
    ...
    if ((2 * n - 1) == ix) {
        double y = yy[2 * n - 2];
        double y2 = y * y;
        double f1 = (shift + y) * (gpd[2 * n - 2] - gpd[n])
                * (xi2 + y2 - 2.0 * x * y) / ((xi2 - y2) * (xi2 - y2));
        double f2 = (shift + x) * (gpd[2 * n - 1] - gpd[n])
                * (xi2 + x2 - 2.0 * x2) / ((xi2 - x2) * (xi2 - x2));
        double f3 = -(shift + 1.0) * gpd[n] * (xi2 + 1.0 - 2.0 * x)
                / ((xi2 - 1.0) * (xi2 - 1.0));
    ...
    if ((2 * n - 2) == ix) {
        double y = yy[2 * n - 1];
        double y2 = y * y;
        double f1 = (shift + x) * (gpd[2 * n - 2] - gpd[n])
                * (xi2 + x2 - 2.0 * x2) / ((xi2 - x2) * (xi2 - x2));
        double f2 = (shift + y) * (gpd[2 * n - 1] - gpd[n])
                * (xi2 + y2 - 2.0 * x * y) / ((xi2 - y2) * (xi2 - y2));
        double f3 = -(shift + 1.0) * gpd[n] * (xi2 + 1.0 - 2.0 * x)
                / ((xi2 - 1.0) * (xi2 - 1.0));
    ...
    if (ix < n) {
        ...
        if (1 == ix) {
            double y = yy[2];
            double f1 = (shift + x) * (gpd[1] - gpd_n) * x
                    / ((xi2 - x2) * (xi2 - x2));
            double f2 = (shift + y) * (gpd[2] - gpd_n) * y
                    / ((xi2 - y * y) * (xi2 - y * y));
            convint += 2.0 * f1 - 0.25 * f2;
        }
        if (2 == ix) {
            double y = yy[1];
            double f1 = (shift + y) * (gpd[1] - gpd_n) * y
                    / ((xi2 - y * y) * (xi2 - y * y));
            double f2 = (shift + x) * (gpd[2] - gpd_n) * x
                    / ((xi2 - x2) * (xi2 - x2));
            convint += 4.0 * f1 + f2;
        }
        ...
        if (ix > 2) {
            ...
            for (i = 2; i < in; i += 2) {
                double y = yy[i];
                double f1 = (shift + y) * (gpd[i] - gpd_n) * y
                        / ((xi2 - y * y) * (xi2 - y * y));
                convint += 2.0 * f1;
            }
            for (i = 1; i < in; i += 2) {
            ...
            if (ix == in) {
                convint += (shift + x) * (gpd[ix] - gpd_n) * x
                        / ((xi2 - x2) * (xi2 - x2));
            }
            if ((ix - 1) == in) {
                double ya = yy[ix - 2];
                double yb = yy[ix - 1];
                double f1 = (shift + ya) * (gpd[ix - 2] - gpd_n) * ya
                        / ((xi2 - ya * ya) * (xi2 - ya * ya));
                double f2 = (shift + yb) * (gpd[ix - 1] - gpd_n) * yb
                        / ((xi2 - yb * yb) * (xi2 - yb * yb));
                double f3 = (shift + x) * (gpd[ix] - gpd_n) * x
                        / ((xi2 - x2) * (xi2 - x2));
                convint += -0.25 * f1 + 3.0 * f2 + 1.25 * f3;
            }
            {
        ...
        for (i = ix + 2; i < in; i += 2) {
            double y = yy[i];
            double f1 = (shift + y) * gpd[i] / ((xi + y) * (xi + y));
            convint += 2.0 * f1;
        }
        for (i = ix + 1; i < in; i += 2) {
            double y = yy[i];
        ...
        if ((2 * n - 1) == in) {
            double y1 = yy[2 * n - 2];
            double y2 = yy[in];
            double f1 = (shift + y1) * gpd[2 * n - 2] / ((xi + y1) * (xi + y1));
            double f2 = (shift + y2) * gpd[2 * n - 1] / ((xi + y2) * (xi + y2));
            convint += -0.25 * f1 + 3.0 * f2;
        }
        {
            // y = x
            convint += (shift + x) * gpd[ix] / ((xi + x) * (xi + x));
        }

        convint *= c_1_3 * gamma;

        convint += -gpd[n] * x2 / (xi2 * (xi + x));
    ...
    if (ix == n) {
        ...
        for (i = ix + 2; i < in; i += 2) {
            double y = yy[i];
            //          double f1 = (shift+y)*(gpd[i]-gpd[n])/((xi+y)*(xi+y));
            double f1 = (shift + y) * gpd[i] / ((xi + y) * (xi + y));
            convint += 2.0 * f1;
        }
        for (i = ix + 1; i < in; i += 2) {
        ...
        if ((2 * n - 1) == in) {
            double y = yy[in];
            double ya = yy[2 * n - 2];
            double f1 = (shift + y) * gpd[in] / ((xi + y) * (xi + y));
            double f1a = (shift + ya) * (gpd[2 * n - 2] - gpd[n])
                    / ((xi + ya) * (xi + ya));
            convint += 3.0 * f1 - 0.25 * f1a;
        }
        {
            // y = x
            convint += (shift + x) * gpd[ix] / (4.0 * xi2);
        }

        convint *= gamma * c_1_3;

        convint += -gpd[n] / (2.0 * xi);
    ...
    if (ix < (2 * n - 2)) {
        ...
        }
        {
            ...
            if ((2 * n - 1) == in) {
                ...
                double f1a = (shift + ya) * (gpd[2 * n - 2] - gpd_n)
                        * (xi2 + ya2 - 2.0 * x * ya)
                        / ((xi2 - ya2) * (xi2 - ya2));
                double f1b = -(shift + 1.0) * gpd_n * (xi2 + 1.0 - 2.0 * x)
                        / ((xi2 - 1.0) * (xi2 - 1.0));
                convint += 2.0 * f1 - 0.25 * f1a + 1.25 * f1b;
            }
        }
        {
            // y = x
            convint += (shift + x) * (gpd[ix] - gpd_n) * (xi2 - x2)
                    / ((xi2 - x2) * (xi2 - x2));
        }
...
int GPDEvolutionVinnikov::gpd_grid_ns(int const n, double const xi,
        std::vector<double>& x) {
    int i;
    ...
    for (i = 0; i <= 2 * n; i++)
        x[i] = -shift * exp(-gamma * (i - 2 * n)) + shift;
    for (i = 2 * n + 1; i <= 4 * n; i++)
        x[i] = shift * exp(gamma * (i - 2 * n)) - shift;
...
int GPDEvolutionVinnikov::gpd_grid_s(int const n, double const xi,
        std::vector<double>& x) {
    int i;
