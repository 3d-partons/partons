...
GPDEvolutionModule::GPDEvolutionModule(const std::string &className) :
        ModuleObject(className, ChannelType::UNDEFINED), m_x(0), m_xi(0), m_t(
                0), m_MuF2(0), m_MuR2(0), m_MuF2_ref(0), m_pertOrder(
                PerturbativeQCDOrderType::UNDEFINED), m_type(
                GPDType::UNDEFINED), m_pRunningAlphaStrongModule(0), m_pActiveFlavorsModule(
                0) {
}
    ...
    if (parameters.isAvailable(
        ...
        info(__func__,
                ElemUtils::Formatter()
                        ...
                        << PerturbativeQCDOrderType(getPertOrder()).toString());
    }
}
...
void GPDEvolutionModule::prepareSubModules(
    ...
    if (it != subModulesData.end()) {
        ...
        m_pRunningAlphaStrongModule->prepareSubModules(
...
void GPDEvolutionModule::setPertOrder(
        const PerturbativeQCDOrderType::Type& pertOrder) {
...
void GPDEvolutionModule::isModuleWellConfigured() {
    ...
    if (m_type == GPDType::UNDEFINED) {
        throw ElemUtils::CustomException(getClassName(), __func__,
                "GPD distribution type is UNDEFINED");
    }
...
PartonDistribution GPDEvolutionModule::compute(const GPDKinematic &kinematic,
        GPDModule* pGPDModule, const GPDType::Type &type) {
