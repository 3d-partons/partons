//#include "../../../../../include/partons/modules/evolution/gpd/VinnikovGPDEvolutionModel.h"
//
...
//                new VinnikovGPDEvolutionModel("VinnikovGPDEvolutionModel"));
//
//VinnikovGPDEvolutionModel::VinnikovGPDEvolutionModel(
...
//                m_nbActiveFlavor / 2.), m_LnMuF2_ref(0.), m_LnMuF2(log(m_MuF2)), m_sizeMuF2Grid(
//                1), m_sizeXGrid(20), m_nonSingletGpd(4 * m_sizeXGrid + 1, 0.), m_singletGpd(
//                2 * m_sizeXGrid + 1, 0.), m_gluonGpd(2 * m_sizeXGrid + 1, 0.), m_singletx(
//                2 * m_sizeXGrid + 1, 0.), m_nonSingletx(4 * m_sizeXGrid + 1, 0.) {
//}
//
//VinnikovGPDEvolutionModel::~VinnikovGPDEvolutionModel() {
//
...
////TODO refactoring
//VinnikovGPDEvolutionModel::VinnikovGPDEvolutionModel(
//        const VinnikovGPDEvolutionModel &other) :
...
//    m_sizeMuF2Grid = other.m_sizeMuF2Grid;
//    m_sizeXGrid = other.m_sizeXGrid;
...
//}
//
//VinnikovGPDEvolutionModel* VinnikovGPDEvolutionModel::clone() const {
//    return new VinnikovGPDEvolutionModel(*this);
//}
//
//void VinnikovGPDEvolutionModel::initModule() {
...
//
////TODO implementer
//void VinnikovGPDEvolutionModel::isModuleWellConfigured() {
...
////}
//
//void VinnikovGPDEvolutionModel::evolution() {
//
//    double Evol;
//
...
//
//        debug(__func__, Formatter() << "iNS = " << iNS);
//
//        evolutionNonSingletGPD(iNS);
//        Evol = interpolate(m_nonSingletx, m_nonSingletGpd, m_x);
//
//        debug(__func__, Formatter() << "Evol NS (interpolated) = " << Evol);
//
...
//}
//
///*In: Current nonSinglet combination iNS
// Out: overwritten grid of GPD values
// Remark: Evolution between MuF2_ref and MuF2 with
// */
//void VinnikovGPDEvolutionModel::evolutionNonSingletGPD(double MuF2Initial,
//        double MuF2Final) {
//
//    // TODO: Take log of scales
//    double LnMuF2Initial = log(MuF2Initial);
//    double LnMuF2Final = log(MuF2Final);
//    // TODO: To think! Ordering of MuF2Initial and MuF2Final?
//
//    double k1[4 * m_sizeXGrid + 1], k2[4 * m_sizeXGrid + 1];
//    double k3[4 * m_sizeXGrid + 1], k4[4 * m_sizeXGrid + 1];
//    double gpd_aux[4 * m_sizeXGrid + 1];
...
//
//    buildNonSingletxGrid(); // Fill vector m_nonSingletx
//
//    initNonSingletGPD(MuF2Initial); // No! Evaluation at muF_ref only. Should be done in mother class.
//
//    // Stupid thing done here to fulfill variable constraints
//    for (unsigned int idummy = 0; idummy < 4 * m_sizeXGrid + 1; idummy++) {
...
//    for (unsigned int j = 0; j <= (m_sizeMuF2Grid - 1); j++) {
//        double t = LnMuF2Initial + j * dt;
//
//        debug(__func__,
...
//        for (unsigned int i = 0; i <= 4 * m_sizeXGrid; i++) {
//            k1[i] = dt
//                    * convolNonSingletKernel(i, m_xi, m_sizeXGrid, dummygpd,
//                            dummyx) * alpha_s(t) * one_twopi;
//
//            debug(__func__, Formatter() << "k1[" << i << "] = " << k1[i]);
//        }
//
//        for (unsigned int i = 0; i <= 4 * m_sizeXGrid; i++) {
...
//        for (unsigned int i = 0; i <= 4 * m_sizeXGrid; i++) {
//            k2[i] = dt
//                    * convolNonSingletKernel(i, m_xi, m_sizeXGrid, gpd_aux,
//                            dummyx) * alpha_s(t + 0.5 * dt) * one_twopi;
//
//            debug(__func__, Formatter() << "k2[" << i << "] = " << k1[i]);
//        }
//
//        for (unsigned int i = 0; i <= 4 * m_sizeXGrid; i++) {
...
//                            dummyx) * alpha_s(t + 0.5 * dt) * one_twopi;
//
//            debug(__func__, Formatter() << "k3[" << i << "] = " << k1[i]);
//        }
//
//        for (unsigned int i = 0; i <= 4 * m_sizeXGrid; i++) {
...
//
//            debug(__func__, Formatter() << "k4[" << i << "] = " << k1[i]);
//        }
//
//        for (unsigned int i = 0; i <= 4 * m_sizeXGrid; i++) {
...
//}
//
//void VinnikovGPDEvolutionModel::evolutionSingletGPD() {
//
//    double k1_q[2 * m_sizeXGrid + 1], k2_q[2 * m_sizeXGrid + 1];
//    double k3_q[2 * m_sizeXGrid + 1], k4_q[2 * m_sizeXGrid + 1];
//    double k1_g[2 * m_sizeXGrid + 1], k2_g[2 * m_sizeXGrid + 1];
...
//    double dummygpd_q[2 * m_sizeXGrid + 1];
//    double dummygpd_g[2 * m_sizeXGrid + 1];
//    double dummyx[2 * m_sizeXGrid + 1];
//
//    buildSingletxGrid();
//
//    initSingletGPD();
//
//    for (unsigned int i = 0; i < 2 * m_sizeXGrid + 1; i++) {
...
//        // ordinary differential equations of first order.
//        // So, Runge-Kutta can be applied.
//        for (unsigned int i = 0; i <= 2 * m_sizeXGrid; i++) {
//            k1_q[i] = dt
//                    * (convolSingletqqKernel(i, m_xi, m_sizeXGrid, dummygpd_q,
//                            dummyx)
//                            + convolSingletqgKernel(i, m_xi, m_sizeXGrid,
//                                    dummygpd_g, dummyx)) * alpha_s(t)
//                    * one_twopi;
//
//            debug(__func__, Formatter() << "k1_q[" << i << "] = " << k1_q[i]);
//
//            k1_g[i] = dt
//                    * (convolSingletgqKernel(i, m_xi, m_sizeXGrid, dummygpd_q,
//                            dummyx)
//                            + convolSingletggKernel(i, m_xi, m_sizeXGrid,
//                                    dummygpd_g, dummyx)) * alpha_s(t)
//                    * one_twopi;
//
//            debug(__func__, Formatter() << "k1_g[" << i << "] = " << k1_g[i]);
//        }
//
//        for (unsigned int i = 0; i <= 2 * m_sizeXGrid; i++) {
...
//        for (unsigned int i = 0; i <= 2 * m_sizeXGrid; i++) {
//            k2_q[i] = dt
//                    * (convolSingletqqKernel(i, m_xi, m_sizeXGrid, gpd_q_aux,
//                            dummyx)
//                            + convolSingletqgKernel(i, m_xi, m_sizeXGrid,
//                                    gpd_g_aux, dummyx)) * alpha_s(t + 0.5 * dt)
//                    * one_twopi;
//
//            debug(__func__, Formatter() << "k2_q[" << i << "] = " << k2_q[i]);
//
//            k2_g[i] = dt
//                    * (convolSingletgqKernel(i, m_xi, m_sizeXGrid, gpd_q_aux,
//                            dummyx)
//                            + convolSingletggKernel(i, m_xi, m_sizeXGrid,
//                                    gpd_g_aux, dummyx)) * alpha_s(t + 0.5 * dt)
//                    * one_twopi;
//
//            debug(__func__, Formatter() << "k2_g[" << i << "] = " << k2_g[i]);
//        }
//
//        for (unsigned int i = 0; i <= 2 * m_sizeXGrid; i++) {
...
//                    * one_twopi;
//
//            debug(__func__, Formatter() << "k3_q[" << i << "] = " << k3_q[i]);
//
//            k3_g[i] = dt
//                    * (convolSingletgqKernel(i, m_xi, m_sizeXGrid, gpd_q_aux,
//                            dummyx)
...
//                    * one_twopi;
//
//            debug(__func__, Formatter() << "k3_g[" << i << "] = " << k3_g[i]);
//        }
//
//        for (unsigned int i = 0; i <= 2 * m_sizeXGrid; i++) {
...
//                    * one_twopi;
//
//            debug(__func__, Formatter() << "k4_q[" << i << "] = " << k4_q[i]);
//
//            k4_g[i] = dt
//                    * (convolSingletgqKernel(i, m_xi, m_sizeXGrid, gpd_q_aux,
//                            dummyx)
...
//                    * one_twopi;
//
//            debug(__func__, Formatter() << "k4_g[" << i << "] = " << k4_g[i]);
//        }
//
//        for (unsigned int i = 0; i <= 2 * m_sizeXGrid; i++) {
...
//}
//
//void VinnikovGPDEvolutionModel::buildNonSingletxGrid() {
//
...
//
//    for (unsigned int i = 0; i <= 2 * m_sizeXGrid; i++) {
//        m_nonSingletx.at(i) = -shift * exp(gamma * (2 * m_sizeXGrid - i))
//                + shift;
//
//        debug(__func__,
...
//    }
//
//    for (unsigned int i = 2 * m_sizeXGrid + 1; i <= 4 * m_sizeXGrid; i++) {
//        m_nonSingletx.at(i) = shift * exp(gamma * (i - 2 * m_sizeXGrid))
//                - shift;
//
//        debug(__func__,
//                Formatter() << "i = " << i << "	m_NonSingletx = "
...
//}
//
//void VinnikovGPDEvolutionModel::buildSingletxGrid() {
//
//    double const gamma = log(
//            0.5 + 0.5 / m_xi * sqrt(9.0 * m_xi * m_xi - 12.0 * m_xi + 4.0))
//            / ((double) m_sizeXGrid);
...
//}
//
//double VinnikovGPDEvolutionModel::alpha_s(double t) {
...
//}
//
//void VinnikovGPDEvolutionModel::initNonSingletGPD() {
//    for (unsigned int iXGrid = 0; iXGrid < m_sizeXGrid; iXGrid++) {
//        m_nonSingletGpd.at(iXGrid) = nonSingletGPD(m_currentNonSingletIndex,
//                m_currentNf, m_nonSingletx.at(iXGrid), m_MuF2);
//
//        debug(__func__,
//                Formatter() << "m_NonSingletGpd.at(" << iXGrid << ") = "
//                        << m_nonSingletGpd.at(iXGrid));
//    }
//}
//
//void VinnikovGPDEvolutionModel::initSingletGPD() {
//
//    for (unsigned int iGrid = 0; iGrid < 2 * m_sizeXGrid + 1; iGrid++) {
//        GPDResult gpdResult = m_pGPDModule->compute(m_singletx.at(iGrid), m_xi,
//                m_t, m_pGPDModule->getMuF2Ref(), m_MuR2,
...
//}
//
//double VinnikovGPDEvolutionModel::convolNonSingletKernel(const unsigned int ix,
...
//}
//
//double VinnikovGPDEvolutionModel::convolSingletqqKernel(const unsigned int ix,
//        const double xi, const int n, const double* const gpd,
//        const double* const yy) {
//
...
//}
//
//double VinnikovGPDEvolutionModel::convolSingletggKernel(const unsigned int ix,
//        const double xi, const int n, const double* const gpd,
//        const double* const yy) {
//
//    // Dimension of "gpd" and "yy" arrays must be 2n+1, i.e. "double gpd[2n+1]"
...
//}
//
//double VinnikovGPDEvolutionModel::convolSingletgqKernel(const unsigned int ix,
//        const double xi, const int n, const double* const gpd,
//        const double* const yy) {
//
//    // Dimension of "gpd" and "yy" arrays must be 2n+1, i.e. "double gpd[2n+1]"
...
//}
//
//unsigned int VinnikovGPDEvolutionModel::getSizeMuF2Grid() const {
//    return m_sizeMuF2Grid;
//}
//
//void VinnikovGPDEvolutionModel::setSizeMuF2Grid(unsigned int sizeMuF2Grid) {
//    m_sizeMuF2Grid = sizeMuF2Grid;
//}
//
//unsigned int VinnikovGPDEvolutionModel::getSizeXGrid() const {
//    return m_sizeXGrid;
//}
//
//void VinnikovGPDEvolutionModel::setSizeXGrid(unsigned int sizeXGrid) {
//    m_sizeXGrid = sizeXGrid;
//
////    m_nonSingletGpd.resize(m_sizeXGrid);
////    m_singletGpd.resize(m_sizeXGrid);
////    m_gluonGpd.resize(m_sizeXGrid);
////    m_singletx.resize(m_sizeXGrid);
////    m_nonSingletx.resize(m_sizeXGrid);
//}
//
//double VinnikovGPDEvolutionModel::integratedNonSingletMuFDerivative(
//        const NfInterval& nfInterval) {
//    evolutionNonSingletGPD(nfInterval.getLowerBound(),
//            nfInterval.getUpperBound());
//
//    return 0.;
//}
//
//double VinnikovGPDEvolutionModel::integratedSingletMuFDerivative(
//        const NfInterval& nfInterval) {
//
//    return 0.;
//}
//
//double VinnikovGPDEvolutionModel::integratedGluonMuFDerivative(
//        const NfInterval& nfInterval) {
//
//    return 0.;
//}
//
//double VinnikovGPDEvolutionModel::discretizedIntegratedNonSingletMuFDerivative(
//        const NfInterval& nfInterval) {
//}
//
//double VinnikovGPDEvolutionModel::discretizedIntegratedSingletMuFDerivative(
//        const NfInterval& nfInterval) {
//}
//
//double VinnikovGPDEvolutionModel::discretizedIntegratedGluonMuFDerivative(
//        const NfInterval& nfInterval) {
//}
//
//double VinnikovGPDEvolutionModel::convolSingletqgKernel(const unsigned int ix,
//        const double xi, const int n, const double* const gpd,
//        const double* const yy) {
//
//    // Dimension of "gpd" and "yy" arrays must be 2n+1, i.e. "double gpd[2n+1]"
...
//}
//
//double VinnikovGPDEvolutionModel::interpolate(std::vector<double> xGrid,
...
//}
//
//void VinnikovGPDEvolutionModel::configure(ParameterList parameters) {
//    if (parameters.isAvailable(
//            VinnikovGPDEvolutionModel::PARAMETER_NAME_SIZE_MUF2_GRID)) {
//        m_sizeMuF2Grid = parameters.getLastAvailable().toUInt();
//
//        info(__func__,
//                Formatter()
//                        << VinnikovGPDEvolutionModel::PARAMETER_NAME_SIZE_MUF2_GRID
//                        << " configured with value = " << m_sizeMuF2Grid);
//    }
//    if (parameters.isAvailable(
//            VinnikovGPDEvolutionModel::PARAMETER_NAME_SIZE_X_GRID)) {
//        m_sizeXGrid = parameters.getLastAvailable().toUInt();
//
//        info(__func__,
//                Formatter()
//                        << VinnikovGPDEvolutionModel::PARAMETER_NAME_SIZE_X_GRID
