#include "VinnikovGPDEvolutionModel.h"
...
void VinnikovGPDEvolutionModel::initNonSingletGPD() {
    for (unsigned int iXGrid = 0; iXGrid < m_sizeXGrid; iXGrid++) {
        m_nonSingletGpd.at(iXGrid) = nonSingletGPD(m_currentNonSingletIndex, m_currentNf,
                m_nonSingletx.at(iXGrid), m_MuF2);
...
double VinnikovGPDEvolutionModel::integratedNonSingletMuFDerivative(
    ...
    evolutionNonSingletGPD(nfInterval.getLowerBound(), nfInterval.getUpperBound());
