...
const std::string CollinearDistributionEvolutionApfel::PARAM_NAME_THRESHOLDS = "thresholds";
const std::string CollinearDistributionEvolutionApfel::PARAM_NAME_MASSES = "masses";
...
const std::string CollinearDistributionEvolutionApfel::PARAM_NAME_TAB_NODES = "tabNodes";
const std::string CollinearDistributionEvolutionApfel::PARAM_NAME_TAB_LOWER_BOUND = "tabLowerBound";
const std::string CollinearDistributionEvolutionApfel::PARAM_NAME_TAB_UPPER_BOUND = "tabUpperBound";
const std::string CollinearDistributionEvolutionApfel::PARAM_NAME_TAB_INTER_DEGREE = "tabInterDegree";
...
CollinearDistributionEvolutionApfel::CollinearDistributionEvolutionApfel(const std::string &className) :
...
void CollinearDistributionEvolutionApfel::configure(const ElemUtils::Parameters &parameters) {
    CollinearDistributionEvolutionModule::configure(parameters);

    if (parameters.isAvailable(CollinearDistributionEvolutionApfel::PARAM_NAME_THRESHOLDS)) {
	...
	info(__func__, ElemUtils::Formatter() << CollinearDistributionEvolutionApfel::PARAM_NAME_THRESHOLDS
	     << " configured with value = [ " << oss.str() << "] GeV");
    }

    if (parameters.isAvailable(CollinearDistributionEvolutionApfel::PARAM_NAME_MASSES)) {
        setMasses(parameters.getLastAvailable().toVectorDouble());
	std::ostringstream oss;
	std::copy(m_masses.begin(), m_masses.end(), std::ostream_iterator<double>(oss, " "));
	info(__func__, ElemUtils::Formatter() << CollinearDistributionEvolutionApfel::PARAM_NAME_MASSES
	     << " configured with value = [ " << oss.str() << "] GeV");
    }

    if (parameters.isAvailable(CollinearDistributionEvolutionApfel::PARAM_NAME_SUBGRID_NODES)) {
	...
	info(__func__, ElemUtils::Formatter() << CollinearDistributionEvolutionApfel::PARAM_NAME_SUBGRID_NODES
	     << " configured with value = [ " << oss.str() << "]");
    }

    if (parameters.isAvailable(CollinearDistributionEvolutionApfel::PARAM_NAME_SUBGRID_LOWER_BOUNDS)) {
	...
	info(__func__, ElemUtils::Formatter() << CollinearDistributionEvolutionApfel::PARAM_NAME_SUBGRID_LOWER_BOUNDS
	     << " configured with value = [ " << oss.str() << "]");
    }

    if (parameters.isAvailable(CollinearDistributionEvolutionApfel::PARAM_NAME_SUBGRID_INTER_DEGREES)) {
	...
	info(__func__, ElemUtils::Formatter() << CollinearDistributionEvolutionApfel::PARAM_NAME_SUBGRID_INTER_DEGREES
	     << " configured with value = [ " << oss.str() << "]");
    }

    if (parameters.isAvailable(CollinearDistributionEvolutionApfel::PARAM_NAME_TAB_NODES)) {
        setTabNodes(parameters.getLastAvailable().toInt());
	info(__func__, ElemUtils::Formatter() << CollinearDistributionEvolutionApfel::PARAM_NAME_TAB_NODES
	     << " configured with value = " << getTabNodes());
    }

    if (parameters.isAvailable(CollinearDistributionEvolutionApfel::PARAM_NAME_TAB_LOWER_BOUND)) {
        setTabLowerBound(parameters.getLastAvailable().toDouble());
	info(__func__, ElemUtils::Formatter() << CollinearDistributionEvolutionApfel::PARAM_NAME_TAB_LOWER_BOUND
	     << " configured with value = " << getTabLowerBound());
    }

    if (parameters.isAvailable(CollinearDistributionEvolutionApfel::PARAM_NAME_TAB_UPPER_BOUND)) {
        setTabUpperBound(parameters.getLastAvailable().toDouble());
	info(__func__, ElemUtils::Formatter() << CollinearDistributionEvolutionApfel::PARAM_NAME_TAB_UPPER_BOUND
	     << " configured with value = " << getTabUpperBound());
    }

    if (parameters.isAvailable(CollinearDistributionEvolutionApfel::PARAM_NAME_TAB_INTER_DEGREE)) {
        setTabInterDegree(parameters.getLastAvailable().toInt());
	info(__func__, ElemUtils::Formatter() << CollinearDistributionEvolutionApfel::PARAM_NAME_TAB_INTER_DEGREE
...
void CollinearDistributionEvolutionApfel::prepareSubModules(const std::map<std::string, BaseObjectData>& subModulesData) {
    ...
    const auto DglapObj = InitializeDglapObjectsQCD(*m_g, m_masses, m_thresholds, true);

    // Construct the DGLAP evolution operators
    const auto EvolvedOps = BuildDglap(DglapObj, this->getMuF_ref(), this->getPertOrder() - 1, as);
...
PartonDistribution CollinearDistributionEvolutionApfel::compute(CollinearDistributionModule* pCollinearDistributionModule) {
    ...
    const std::map<int, apfel::Distribution> topsdist = apfel::QCDEvToPhys((tops * apfel::Set<apfel::Distribution> {apfel::EvolveDistributionsBasisQCD{}, DistributionMap(*m_g, initialScaleDistributions(pCollinearDistributionModule))}).GetObjects());
...
std::function<std::map<int, double>(double const&)> CollinearDistributionEvolutionApfel::initialScaleDistributions(CollinearDistributionModule* pCollinearDistributionModule) {
...
void CollinearDistributionEvolutionApfel::setThresholds(const std::vector<double>& thresholds) {
  m_thresholds = thresholds;
}

void CollinearDistributionEvolutionApfel::setMasses(const std::vector<double>& masses) {
  m_masses = masses;
}

void CollinearDistributionEvolutionApfel::setSubgridNodes(const std::vector<int>& subgridNodes) {
  m_subgridNodes = subgridNodes;
}

void CollinearDistributionEvolutionApfel::setSubgridLowerBounds(const std::vector<double>& subgridLowerBounds) {
  m_subgridLowerBounds = subgridLowerBounds;
}

void CollinearDistributionEvolutionApfel::setSubgridInterDegrees(const std::vector<int>& subgridInterDegrees) {
...
void CollinearDistributionEvolutionApfel::setTabLowerBound(const double& tabLowerBound) {
  m_tabLowerBound = tabLowerBound;
}

void CollinearDistributionEvolutionApfel::setTabUpperBound(const double& tabUpperBound) {
  m_tabUpperBound = tabUpperBound;
}

void CollinearDistributionEvolutionApfel::setTabInterDegree(const int& tabInterDegree) {
...
CollinearDistributionEvolutionApfel::CollinearDistributionEvolutionApfel(const CollinearDistributionEvolutionApfel &other) :
