...
GPDVinnikov06::~GPDVinnikov06() {
}

GPDVinnikov06* GPDVinnikov06::clone() const {
    return new GPDVinnikov06(*this);
}

void GPDVinnikov06::resolveObjectDependencies() {
}

void GPDVinnikov06::configure(const ElemUtils::Parameters &parameters) {
    GPDModule::configure(parameters);
}

std::string GPDVinnikov06::toString() const {
    return GPDModule::toString();
}

GPDVinnikov06::GPDVinnikov06(const GPDVinnikov06& other) :
        EPS_BETR(1.E-8), SIMP_INT(100), GPDModule(other) {
...
void GPDVinnikov06::isModuleWellConfigured() {
    GPDModule::isModuleWellConfigured();
}

void GPDVinnikov06::initModule() {
    GPDModule::initModule();
}

PartonDistribution GPDVinnikov06::computeH() {
...
PartonDistribution GPDVinnikov06::computeHt() {
...
double GPDVinnikov06::gpdh(int const i_part, double x, double const xi,
    ...
    if (x < -xi) {
        ...
        double h = dd_int_simp(bmin, bmax, &GPDVinnikov06::dd1, i_part, x, xi,
                t, n_par, param);
        return h / xi;
    }
    ...
    if (x <= 0.0) {
        ...
        double h1 = dd_int_simp(0.0, bmax1, &GPDVinnikov06::dd2, i_part, x, xi,
                t, n_par, param);
        double h2 = dd_int_simp(bmax1, bmax2, &GPDVinnikov06::dd1, i_part, x,
                xi, t, n_par, param);
        return (h1 + h2) / xi;
    }
    if (x < xi) {
        ...
        double h1 = dd_int_simp(0.0, bmax1, &GPDVinnikov06::dd2, i_part, x, xi,
                t, n_par, param);
        double h2 = dd_int_simp(bmax1, bmax2, &GPDVinnikov06::dd3, i_part, x,
                xi, t, n_par, param);
        return (h1 + h2) / xi;
    }
    if (x <= 1.0) {
        double bmin = (x - xi) / (1.0 - xi);
        double bmax = (x + xi) / (1.0 + xi);
        double h = dd_int_simp(bmin, bmax, &GPDVinnikov06::dd3, i_part, x, xi,
                t, n_par, param);
        return h / xi;
    }
    if ((x - EPS_BETR) <= 1.0)
...
double GPDVinnikov06::dd1(int const i_part, double const beta, double const x,
...
double GPDVinnikov06::dd2(int const i_part, double const beta, double const x,
        double const xi, double const t, int const n_par, double* const param) {
...
double GPDVinnikov06::dd3(int const i_part, double const beta, double const x,
        double const xi, double const t, int const n_par, double* const param) {
    double prof_b_val = param[n_par * abs(i_part)];
    double prof_b_sea = param[n_par * (abs(i_part) + 2)];
...
double GPDVinnikov06::uval(double const x, double const t) {
    if (x <= 0.0) {
...
double GPDVinnikov06::usea(double const x, double const t) {
    if (x <= 0.0 || x > 1.0) {
        throw ElemUtils::CustomException(getClassName(), __func__,
                ElemUtils::Formatter() << "x = " << x << " < 0");
    }
...
double GPDVinnikov06::dval(double const x, double const t) {
    if (x <= 0.0 || x > 1.0) {
        throw ElemUtils::CustomException(getClassName(), __func__,
                ElemUtils::Formatter() << "x = " << x << " < 0");
    }
...
double GPDVinnikov06::dsea(double const x, double const t) {
    if (x <= 0.0 || x > 1.0) {
        throw ElemUtils::CustomException(getClassName(), __func__,
                ElemUtils::Formatter() << "x = " << x << " < 0");
    }
...
double GPDVinnikov06::ssea(double const x, double const t) {
    if (x <= 0.0 || x > 1.0) {
        throw ElemUtils::CustomException(getClassName(), __func__,
                ElemUtils::Formatter() << "x = " << x << " < 0");
    }
...
double GPDVinnikov06::glu(double const x, double const t) {
    if (x <= 0.0 || x > 1.0) {
        throw ElemUtils::CustomException(getClassName(), __func__,
                ElemUtils::Formatter() << "x = " << x << " < 0");
    }
...
double GPDVinnikov06::gpdh_pol(int const i_part, double x, double const xi,
        double const t, int const n_par, double* const param) {
    if (x == xi) {
        x -= EPS_BETR;
    }
    ...
    if (x < -xi) {
        double bmin = (-x - xi) / (1.0 - xi);
        double bmax = (-x + xi) / (1.0 + xi);
        double h = dd_int_simp(bmin, bmax, &GPDVinnikov06::dd1_pol, i_part, x,
                xi, t, n_par, param);
        return h / xi;
    }
    ...
    if (x <= 0.0) {
        double bmax1 = (xi + x) / (1.0 + xi);
        double bmax2 = (xi - x) / (1.0 + xi);
        double h1 = dd_int_simp(0.0, bmax1, &GPDVinnikov06::dd2_pol, i_part, x,
                xi, t, n_par, param);
        double h2 = dd_int_simp(bmax1, bmax2, &GPDVinnikov06::dd1_pol, i_part,
                x, xi, t, n_par, param);
        return (h1 + h2) / xi;
    }
    if (x < xi) {
        ...
        double h1 = dd_int_simp(0.0, bmax1, &GPDVinnikov06::dd2_pol, i_part, x,
                xi, t, n_par, param);
        double h2 = dd_int_simp(bmax1, bmax2, &GPDVinnikov06::dd3_pol, i_part,
                x, xi, t, n_par, param);
        return (h1 + h2) / xi;
    }
    if (x <= 1.0) {
        double bmin = (x - xi) / (1.0 - xi);
        double bmax = (x + xi) / (1.0 + xi);
        double h = dd_int_simp(bmin, bmax, &GPDVinnikov06::dd3_pol, i_part, x,
                xi, t, n_par, param);
        return h / xi;
    }
    if ((x - EPS_BETR) <= 1.0) {
...
double GPDVinnikov06::dd1_pol(int const i_part, double const beta,
...
double GPDVinnikov06::dd2_pol(int const i_part, double const beta,
        double const x, double const xi, double const t, int const n_par,
        double* const param) {
    double prof_b_val = param[n_par * abs(i_part)];
    double prof_b_sea = param[n_par * (abs(i_part) + 2)];
...
double GPDVinnikov06::dd3_pol(int const i_part, double const beta,
        double const x, double const xi, double const t, int const n_par,
        double* const param) {
    double prof_b_val = param[n_par * abs(i_part)];
    double prof_b_sea = param[n_par * (abs(i_part) + 2)];
...
double GPDVinnikov06::uval_pol(double const x, double const t) {
    if (x <= 0.0) {
        throw ElemUtils::CustomException(getClassName(), __func__,
                ElemUtils::Formatter() << "x = " << x << " < 0");
    }
...
double GPDVinnikov06::usea_pol(double const x, double const t) {
    if (x <= 0.0 || x > 1.0) {
        throw ElemUtils::CustomException(getClassName(), __func__,
                ElemUtils::Formatter() << "x = " << x << " < 0");
    }
...
double GPDVinnikov06::dval_pol(double const x, double const t) {
    if (x <= 0.0 || x > 1.0) {
        throw ElemUtils::CustomException(getClassName(), __func__,
                ElemUtils::Formatter() << "x = " << x << " < 0");
    }
...
double GPDVinnikov06::dsea_pol(double const x, double const t) {
    if (x <= 0.0 || x > 1.0) {
        throw ElemUtils::CustomException(getClassName(), __func__,
                ElemUtils::Formatter() << "x = " << x << " < 0");
    }
...
double GPDVinnikov06::ssea_pol(double const x, double const t) {
    if (x <= 0.0 || x > 1.0) {
        throw ElemUtils::CustomException(getClassName(), __func__,
                ElemUtils::Formatter() << "x = " << x << " < 0");
    }
...
double GPDVinnikov06::glu_pol(double const x, double const t) {
    if (x <= 0.0 || x > 1.0) {
        throw ElemUtils::CustomException(getClassName(), __func__,
                ElemUtils::Formatter() << "x = " << x << " < 0");
    }
...
double GPDVinnikov06::prof_funct(double const b, double const beta,
        double const alpha) {
...
double GPDVinnikov06::power(double const x, double const y) {
    return exp(y * log(x));
}

double GPDVinnikov06::gammf(double xx) {
    double x, y, tmp, ser;
...
double GPDVinnikov06::dd_int_simp(double const bmin, double const bmax,
        double (GPDVinnikov06::*dd)(int, double, double, double, double, int,
