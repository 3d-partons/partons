...
GPDGK16Numerical::GPDGK16Numerical(const GPDGK16Numerical& other) :
...
GPDGK16Numerical* GPDGK16Numerical::clone() const {
    return new GPDGK16Numerical(*this);
}
...
//TODO implement
void GPDGK16Numerical::isModuleWellConfigured() {
    GPDModule::isModuleWellConfigured();

}

void GPDGK16Numerical::initModule() {
...
//Profile function
double GPDGK16Numerical::Profile(double N, double beta, double alpha) {
    double profile = 0.;
...
double GPDGK16Numerical::GKPdfAnsatz(double N, double beta, double* coeff) {
...
double GPDGK16Numerical::GKPdfAnsatz_t(double N, double beta, double* coeff) {
// as in arXiv:1210.6975v3 pag 9
    return pow(beta, -coeff[4]) * pow((1. - beta), 2 * N + 1)
            * (coeff[0] + coeff[1] * beta + coeff[2] * beta * beta) * coeff[3];
}

double GPDGK16Numerical::GKPdfAnsatz_tg(double N, double beta, double* coeff) {
// as in arXiv:1210.6975v3 pag 9
    return pow(beta, -coeff[4]) * pow((1. - beta), 2 * N + 1)
...
//DD for H
double GPDGK16Numerical::GKtDependentsDD(double beta, double alpha) {
...
double GPDGK16Numerical::GKtDependentuValDD(double beta, double alpha) {
    double absbeta = fabs(beta);
    double uValDD;
    /*    if (beta <= 0 || beta > 1.) {
     throwBetaException(__func__, x);
...
double GPDGK16Numerical::GKtDependentdValDD(double beta, double alpha) {
    double absbeta = fabs(beta);
    double dValDD;
    /*    if (beta <= 0 || beta > 1.) {
     throwBetaException(__func__, x);
...
double GPDGK16Numerical::GKtDependentgDD(double beta, double alpha) {
    double absbeta = fabs(beta);
    /*    if (beta <= 0 || beta > 1.) {
     throwBetaException(__func__, x);
     }*/
...
double GPDGK16Numerical::GKtDependentuDD_t(double beta, double alpha) {
    double absbeta = fabs(beta);
    /*    if (beta <= 0 || beta > 1.) {
     throwBetaException(__func__, x);
     }*/
...
double GPDGK16Numerical::GKtDependentdDD_t(double beta, double alpha) {
    double absbeta = fabs(beta);
    /*    if (beta <= 0 || beta > 1.) {
     throwBetaException(__func__, x);
     }*/
...
double GPDGK16Numerical::GKtDependentHtgDD(double beta, double alpha) {
...
//DD for E
double GPDGK16Numerical::GKtDependentDD_EuVal(double beta, double alpha) {
    double absbeta = fabs(beta);
...
double GPDGK16Numerical::GKtDependentDD_EdVal(double beta, double alpha) {
    double absbeta = fabs(beta);
...
double GPDGK16Numerical::GKtDependentDD_Es(double beta, double alpha) {
    double absbeta = fabs(beta);
    double Es;
    double betas = 7;
    double Ns = -0.155;
...
double GPDGK16Numerical::GKtDependentDD_Eg(double beta, double alpha) {
    double absbeta = fabs(beta);
    double Eg;
    double betag = 6;
    double Ng = 0.779;
...
//DDs for Et
double GPDGK16Numerical::GKtDependentDD_EtuVal(double beta, double alpha) {
    double absbeta = fabs(beta);
    double EtuVal;
    double N = 14;
    alphaP = 0.45;
...
double GPDGK16Numerical::GKtDependentDD_EtdVal(double beta, double alpha) {
    double absbeta = fabs(beta);
    double EduVal;
    double N = 4;
    alphaP = 0.45;
...
double GPDGK16Numerical::IntegralEtuVal(double beta, std::vector<double> Par) {
...
double GPDGK16Numerical::IntegralEtdVal(double beta, std::vector<double> Par) {
    double alpha = (m_x - beta) / m_xi;
...
double GPDGK16Numerical::IntegralEtuValMx(double beta, std::vector<double> Par) {
...
double GPDGK16Numerical::IntegralEtdValMx(double beta, std::vector<double> Par) {
    double alpha = (m_Mx - beta) / m_xi;
...
//Integrals for GPD E
double GPDGK16Numerical::IntegralEuVal(double beta, std::vector<double> Par) {
    double alpha = (m_x - beta) / m_xi;
...
double GPDGK16Numerical::IntegralEdVal(double beta, std::vector<double> Par) {
    double alpha = (m_x - beta) / m_xi;
...
double GPDGK16Numerical::IntegralEuValMx(double beta, std::vector<double> Par) {
    double alpha = (m_Mx - beta) / m_xi;
...
double GPDGK16Numerical::IntegralEdValMx(double beta, std::vector<double> Par) {
    double alpha = (m_Mx - beta) / m_xi;
...
double GPDGK16Numerical::IntegralxLargeEsSea(double beta,
...
double GPDGK16Numerical::IntegralxLargeEsSeaMx(double beta,
        std::vector<double> Par) {
    double alpha = (m_x + beta) / m_xi;
...
double GPDGK16Numerical::IntegralxSmallEsSea(double beta,
        std::vector<double> Par) {
    return (GKtDependentDD_Es(beta, (m_x - beta) / m_xi)
            - GKtDependentDD_Es(beta, (m_x + beta) / m_xi)) / m_xi;
}

double GPDGK16Numerical::IntegralxSmall1EsSea(double beta,
        std::vector<double> Par) {
    double alpha = (m_x - beta) / m_xi;
...
double GPDGK16Numerical::IntegralxSmall2EsSea(double beta,
        std::vector<double> Par) {
    double Integral;
...
double GPDGK16Numerical::IntegralxLargeEgSea(double beta,
        std::vector<double> Par) {
    double alpha = (m_x - beta) / m_xi;
...
double GPDGK16Numerical::IntegralxLargeEgSeaMx(double beta,
        std::vector<double> Par) {
    double alpha = (m_x + beta) / m_xi;
...
double GPDGK16Numerical::IntegralxSmall1EgSea(double beta,
        std::vector<double> Par) {
    double alpha = (m_x - beta) / m_xi;
...
double GPDGK16Numerical::IntegralxSmall2EgSea(double beta,
        std::vector<double> Par) {
    double alpha = (m_x + beta) / m_xi;
...
//inetgrals for H
double GPDGK16Numerical::IntegralHuVal(double beta, std::vector<double> Par) {
    double alpha = (m_x - beta) / m_xi;
...
double GPDGK16Numerical::IntegralHuValMx(double beta, std::vector<double> Par) {
    double alpha = (m_Mx - beta) / m_xi;
...
double GPDGK16Numerical::IntegralHdVal(double beta, std::vector<double> Par) {
    double alpha = (m_x - beta) / m_xi;
...
double GPDGK16Numerical::IntegralHdValMx(double beta, std::vector<double> Par) {
    double alpha = (m_Mx - beta) / m_xi;
...
double GPDGK16Numerical::IntegralxLargeHg(double beta, std::vector<double> Par) {
    double alpha = (m_x - beta) / m_xi;
...
double GPDGK16Numerical::IntegralxLargeHgMx(double beta, std::vector<double> Par) {
    double alpha = (m_x + beta) / m_xi;
...
double GPDGK16Numerical::IntegralxSmall1Hg(double beta, std::vector<double> Par) {
    double alpha = (m_x - beta) / m_xi;
...
double GPDGK16Numerical::IntegralxSmall2Hg(double beta, std::vector<double> Par) {
    double alpha = (m_x + beta) / m_xi;
...
double GPDGK16Numerical::IntegralxLargeHsSea(double beta,
        std::vector<double> Par) {
    double alpha = (m_x - beta) / m_xi;
...
double GPDGK16Numerical::IntegralxLargeHsSeaMx(double beta,
        std::vector<double> Par) {
    double alpha = (m_x + beta) / m_xi;
...
double GPDGK16Numerical::IntegralxSmallHsSea(double beta,
        std::vector<double> Par) {
    return (GKtDependentsDD(beta, (m_x - beta) / m_xi)
            - GKtDependentsDD(beta, (m_x + beta) / m_xi)) / m_xi;
}

double GPDGK16Numerical::IntegralxSmall1HsSea(double beta,
        std::vector<double> Par) {
    double alpha = (m_x - beta) / m_xi;
...
double GPDGK16Numerical::IntegralxSmall2HsSea(double beta,
        std::vector<double> Par) {
    double Integral;
...
//Integrals for Ht
double GPDGK16Numerical::IntegralHtuVal(double beta, std::vector<double> Par) {
    double alpha = (m_x - beta) / m_xi;
...
double GPDGK16Numerical::IntegralHtdVal(double beta, std::vector<double> Par) {
    double alpha = (m_x - beta) / m_xi;
...
double GPDGK16Numerical::IntegralHtuValMx(double beta, std::vector<double> Par) {
    double alpha = (m_Mx - beta) / m_xi;
...
double GPDGK16Numerical::IntegralHtdValMx(double beta, std::vector<double> Par) {
    double alpha = (m_Mx - beta) / m_xi;
...
double GPDGK16Numerical::IntegralxLargeHtg(double beta, std::vector<double> Par) {
    double alpha = (m_x - beta) / m_xi;
...
double GPDGK16Numerical::IntegralxLargeHtgMx(double beta,
        std::vector<double> Par) {
    double alpha = (m_Mx - beta) / m_xi;
...
double GPDGK16Numerical::IntegralxSmall1Htg(double beta, std::vector<double> Par) {
    double alpha = (m_x - beta) / m_xi;
...
double GPDGK16Numerical::IntegralxSmall2Htg(double beta, std::vector<double> Par) {
    double alpha = (m_Mx - beta) / m_xi;
...
PartonDistribution GPDGK16Numerical::computeH() {
...
PartonDistribution GPDGK16Numerical::computeHt() {
...
PartonDistribution GPDGK16Numerical::computeE() {
    PartonDistribution partonDistribution;
...
PartonDistribution GPDGK16Numerical::computeEt() {
    PartonDistribution partonDistribution;
...
double GPDGK16Numerical::Et_pole(double x) {
