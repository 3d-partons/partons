...
const std::string GPDMMS13::PARAMETER_NAME_MMS13MODEL_NHpE = "MMS13Model_NHpE";
const std::string GPDMMS13::PARAMETER_NAME_MMS13MODEL_NE = "MMS13Model_NE";
...
GPDMMS13::GPDMMS13(const GPDMMS13& other) :
...
void GPDMMS13::configure(const ElemUtils::Parameters &parameters) {
    ...
    if (parameters.isAvailable(GPDMMS13::PARAMETER_NAME_MMS13MODEL_NE)) {
        m_NE = parameters.getLastAvailable().toInt();
    }

    if (parameters.isAvailable(GPDMMS13::PARAMETER_NAME_MMS13MODEL_C)) {
        m_C = parameters.getLastAvailable().toDouble();
    }
}

std::string GPDMMS13::toString() const {
...
PartonDistribution GPDMMS13::computeE() {
...
double GPDMMS13::forwardHval(double beta, QuarkFlavor::Type flavor) const {
...
double GPDMMS13::forwardEval(double beta, QuarkFlavor::Type flavor) const {
...
double GPDMMS13::forwardEvalFunction(double beta, double kappa, double nu,
        double mu) const {
...
double GPDMMS13::HpEDDVal(double x, QuarkFlavor::Type flavor, int N) {
...
double GPDMMS13::EValPlus(double x, QuarkFlavor::Type flavor, int N) {
...
double GPDMMS13::IntHpEDDval(double beta, std::vector<double> par) {
    return (1 / m_xi)
...
double GPDMMS13::IntEvalPlusAB(double beta, std::vector<double> par) {
...
double GPDMMS13::IntEvalPlusA(double beta, std::vector<double> par) {
    return (1 / m_xi)
            * forwardEval(beta, static_cast<QuarkFlavor::Type>(int(par[1])))
            * profileFunction(beta, (par[0] - beta) / m_xi, int(par[2])) / beta;
}

double GPDMMS13::IntEvalPlusB(double beta, std::vector<double> par) {
    return (1 / m_xi)
            * forwardEval(beta, static_cast<QuarkFlavor::Type>(int(par[1])))
