...
VinnikovEvolQCDModel::VinnikovEvolQCDModel() :
		...
		m_nbColor( 3. ),
		m_CF( 0.5* ( m_nbColor - 1. / m_nbColor ) ),
		m_CA( 2. * m_CF + 1. / m_nbColor ),
		m_nbActiveFlavor( 3. ),
		m_b0( 11. * m_nbColor / 3. - 2. * m_nbActiveFlavor / 3. ),
		m_TR( m_nbActiveFlavor / 2. ),
		m_LnMuF2_ref( log(m_MuF_ref*m_MuF_ref) ),
		m_LnMuF2(log(m_MuF*m_MuF)),
		m_NonSingletGpd(4*m_nbXPoints+1, 0.),
		m_SingletGpd(2*m_nbXPoints+1, 0.),
		m_GluonGpd(2*m_nbXPoints+1, 0.),
		m_Singletx(2*m_nbXPoints+1, 0.),
		m_NonSingletx(4*m_nbXPoints+1, 0.) {
}
...
void VinnikovEvolQCDModel::evolutionNonSingletGPD(unsigned int iNS) {
	...
	for (unsigned int j=0; j <=(m_nbMuFPoints-1); j++) {
		...
		for (unsigned int i=0; i<=4*m_nbXPoints; i++) {
			k1[i] = dt
					*convolNonSingletKernel(i, m_xi,m_nbXPoints,dummygpd,dummyx)
					*alpha_s(t)*one_twopi;
		...
		for (unsigned int i=0; i<=4*m_nbXPoints; i++) {
			k2[i] = dt
				*convolNonSingletKernel(i, m_xi, m_nbXPoints, gpd_aux, dummyx)
				*alpha_s(t+0.5*dt)*one_twopi;
		...
		for (unsigned int i=0; i<=4*m_nbXPoints; i++) {
			m_NonSingletGpd.at(i) += (k1[i] +2.0*k2[i] +2.0*k3[i] +k4[i])/6.0;
...
void VinnikovEvolQCDModel::evolutionSingletGPD() {
	...
	for (unsigned int i = 0; i < 2*m_nbXPoints+1 ; i ++) {
		...
		m_pLoggerManager->debug(getClassName(), __func__,
				...
				<< "	initial GPDS = " << m_SingletGpd.at(i)
				<< "	initial GPD gluon = " << m_GluonGpd.at(i));
	}
	...
	for (unsigned int j=0; j<m_nbMuFPoints; j++) {
		...
		for (unsigned int i=0; i<=2*m_nbXPoints; i++) {
			k1_q[i] = dt
				*(convolSingletqqKernel(i, m_xi, m_nbXPoints, dummygpd_q, dummyx)
				+convolSingletqgKernel(i, m_xi, m_nbXPoints, dummygpd_g, dummyx))
				*alpha_s(t)*one_twopi;
			...
			k1_g[i] = dt
				*(convolSingletgqKernel(i, m_xi, m_nbXPoints, dummygpd_q, dummyx)
				+convolSingletggKernel(i, m_xi, m_nbXPoints, dummygpd_g, dummyx))
				*alpha_s(t)*one_twopi;
		...
		for (unsigned int i=0; i<=2*m_nbXPoints; i++) {
			...
			m_pLoggerManager->debug(getClassName(), __func__,
				Formatter() << "gpd_q_aux k1[" << i << "] = " << gpd_q_aux[i]
				<< "	gpd_g_aux k1[" << i << "] = " << gpd_g_aux[i]);
		}

		for (unsigned int i=0; i<=2*m_nbXPoints; i++) {
			k2_q[i] = dt
				*(convolSingletqqKernel(i, m_xi, m_nbXPoints, gpd_q_aux, dummyx)
				+convolSingletqgKernel(i, m_xi, m_nbXPoints, gpd_g_aux, dummyx))
				*alpha_s(t+0.5*dt)*one_twopi;
			...
			k2_g[i] = dt
				*(convolSingletgqKernel(i, m_xi, m_nbXPoints, gpd_q_aux, dummyx)
				+convolSingletggKernel(i, m_xi, m_nbXPoints, gpd_g_aux, dummyx))
				*alpha_s(t+0.5*dt)*one_twopi;
		...
		for (unsigned int i=0; i<=2*m_nbXPoints; i++) {
			...
			m_pLoggerManager->debug(getClassName(), __func__,
				Formatter() << "gpd_q_aux k2[" << i << "] = " << gpd_q_aux[i]
				<< "	gpd_g_aux k2[" << i << "] = " << gpd_g_aux[i]);
		}
		...
		for (unsigned int i=0; i<=2*m_nbXPoints; i++) {
			...
			m_pLoggerManager->debug(getClassName(), __func__,
				Formatter() << "gpd_q_aux k3[" << i << "] = " << gpd_q_aux[i]
				<< "	gpd_g_aux k3[" << i << "] = " << gpd_g_aux[i]);
		}
		...
		for (unsigned int i=0; i<=2*m_nbXPoints; i++) {
			m_SingletGpd.at(i) += (k1_q[i] +2.0*k2_q[i] +2.0*k3_q[i] +k4_q[i])/6.0;
			m_GluonGpd.at(i) += (k1_g[i] +2.0*k2_g[i] +2.0*k3_g[i] +k4_g[i])/6.0;
...
void VinnikovEvolQCDModel::buildNonSingletxGrid() {
	...
	double const gamma = log( 0.5+0.5/m_xi*sqrt( 9.0*m_xi*m_xi-12.0*m_xi+4.0 ) )
			/ ( m_nbXPoints );
	double const shift = m_xi*m_xi/(1.0-2.0*m_xi);

	for (unsigned int i=0;i<=2*m_nbXPoints;i++) {
		m_NonSingletx.at(i) = -shift*exp( gamma*(2*m_nbXPoints-i) ) + shift;

		m_pLoggerManager->debug(getClassName(), __func__,
				Formatter() << "i = " << i << "	m_NonSingletx = " << m_NonSingletx.at(i));
	}

	for (unsigned int i=2*m_nbXPoints+1;i<=4*m_nbXPoints;i++) {
		m_NonSingletx.at(i) = shift*exp(gamma*(i-2*m_nbXPoints)) - shift;
...
void VinnikovEvolQCDModel::buildSingletxGrid() {
	...
	for (unsigned int i=0;i<=2*m_nbXPoints;i++) {
		...
		m_pLoggerManager->debug(getClassName(), __func__,
			Formatter() << "i = " << i << "	m_Singletx = " << m_Singletx.at(i));
	}
...
double VinnikovEvolQCDModel::alpha_s(double t) {
	...
	if (m_nbActiveFlavor == 3) L = t - 2.0*LOG_LAMBDA_3;
	else if (m_nbActiveFlavor == 4) L = t - 2.0*LOG_LAMBDA_4;
	else if (m_nbActiveFlavor == 5) L = t - 2.0*LOG_LAMBDA_5;
	else {
		printf("WARNING (alpha_s.c): m_nbActiveFlavor not between 3 and 5. Returning alpha_s = 0\n");
		return 0.;
	}

	if ((alpha = 4.0*M_PI/(B0*L)*(1.0 - 2.0*B1*log(L)/(B0*B0*L))) > 1.0) {
...
void VinnikovEvolQCDModel::initNonSingletGPD(unsigned int iNS) {
	...
	for (unsigned int iGrid = 0 ; iGrid < 4*m_nbXPoints+1 ; iGrid ++ ) {
		...
		pGpdResultData = gpdOutputData.getGPDResultData(m_currentGPDComputeType);

		m_pLoggerManager->debug(getClassName(), __func__,
				Formatter() << "GpdResultData = " << pGpdResultData->toString());

		vectorOfGPDCombinations
			= EvolQCDModule::MakeVectorOfGPDCombinations(*pGpdResultData);
		// TODO: A whole matrix product is computed while only one line is used... Improve!
		vectorOfGPDCombinations
			= EvolQCDModule::convertBasis(vectorOfGPDCombinations);
...
void VinnikovEvolQCDModel::initSingletGPD() {
	...
	for (unsigned int iGrid = 0 ; iGrid < 2*m_nbXPoints+1 ; iGrid ++) {
		GPDOutputData gpdOutputData = m_pGPDModule->compute(m_Singletx.at(iGrid),
				m_xi, m_t, m_MuF_ref, m_MuR, m_currentGPDComputeType);
		GPDResultData* pGpdResultData = gpdOutputData.getGPDResultData(m_currentGPDComputeType);
		m_GluonGpd.at(iGrid) = pGpdResultData->getGluon();
		m_SingletGpd.at(iGrid) = pGpdResultData->getSinglet();
	}
}

double VinnikovEvolQCDModel::convolNonSingletKernel(const unsigned int ix, const double xi,
		const int n, const double* const gpd, const double* const yy) {
	...
	if(0 == ix) return 0.0;
	if(4*n == ix) return 0.0;

	if(1 == ix) {
		double y = yy[2];
		double x2m2xi2 = x2 - 2.0*xi2;
		double f1 = (shift+1.0)*gpd[1]*(x2m2xi2 + 1.0)/((-1.0 - x)*(1.0 - xi2));
		double f2 = - ( - 3.0*gpd[1] + 6.0*gpd[2] - gpd[3] ) /(3.0*gamma);
		double f3 = (shift-y)*(gpd[1] - gpd[2])*(x2m2xi2 + y*y)
				/((y - x)*(y*y - xi2));
		return m_CF*( c_1_3*gamma*(1.25*f1 + 2.0*f2 - 0.25*f3)
				+ gpd[1]*(1.5 + 2.0*log(1.0+x)+ 0.5*(x-xi)/xi
				*log((xi-x)*(1.0-xi))
	...
	if(2 == ix) {
		double y = yy[1];
		double x2m2xi2 = x2 - 2.0*xi2;
		double f1 = (shift+1.0)*gpd[2]*(x2m2xi2 + 1.0)/((-1.0 - x)*(1.0 - xi2));
		double f2 = (shift-y)*(gpd[2] - gpd[1])*(x2m2xi2 + y*y)/
				((y - x)*(y*y - xi2));
		double f3 = - ( - 6.0*gpd[1] + 3.0*gpd[2] + 2.0*gpd[3] )/(3.0*gamma);
		return m_CF*(c_1_3*gamma*(f1 + 4.0*f2 + f3)
				+ gpd[2]*(1.5 + 2.0*log(1.0+x)
				+ 0.5*(x-xi)/xi*log((xi-x)*(1.0-xi))
				- 0.5*(x+xi)/xi*log((-xi-x)*(1.0+xi)) ) );
	}
	...
	if((4*n-1) == ix) {
		double y = yy[4*n-2];
		double f1 = (shift+y)*(gpd[4*n-2] - gpd[ix])*(y*y + x2 - 2.0*xi2)/
				((y - x)*(y*y - xi2));
		double f2 = ( gpd[ix-2] - 6.0*gpd[ix-1] + 3.0*gpd[ix])/(3.0*gamma);
		double f3 = -(shift+1.0)*gpd[ix]*(1.0 + x2 - 2.0*xi2)
				/((1.0 - x)*(1.0 - xi2));
		return m_CF*(c_1_3*gamma*(-0.25*f1 + 2.0*f2 + 1.25*f3)
				+ gpd[1]*(3.0/2.0 + 2.0*log(1.0-x)+ 0.5*(x-xi)/xi
				*log((x-xi)*(1.0+xi)) - 0.5*(x+xi)/xi*log((xi+x)*(1.0-xi)) ) );
	}

	if((4*n-2) == ix) {
		double y = yy[4*n-1];
		double f1 = ( -2.0*gpd[ix-1] - 3.0*gpd[ix] + 6.0*gpd[ix+1])/(3.0*gamma);
		double f2 = (shift+y)*(gpd[4*n-1] - gpd[ix])*(y*y + x2 - 2.0*xi2)/
				((y - x)*(y*y - xi2));
		double f3 = -(shift+1.0)*gpd[ix]*(1.0 + x2 - 2.0*xi2)
				/((1.0 - x)*(1.0 - xi2));
		return m_CF*(c_1_3*gamma*(f1 + 4.0*f2 + f3)
				+ gpd[2]*(3.0/2.0 + 2.0*log(1.0-x) + 0.5*(x-xi)/xi
				*log((x-xi)*(1.0+xi)) - 0.5*(x+xi)/xi*log((xi+x)*(1.0-xi)) ) );
	}
	...
	if(ix < n) {
		...
		if((ix-1) == in) {
			double ya = yy[ix-2];
			double yb = yy[ix-1];
			double f1 = (shift-ya)*(gpd_x - gpd[ix-2])*(x2m2xi2 + ya*ya)/
					((ya - x)*(ya*ya - xi2));
			double f2 = (shift-yb)*(gpd_x - gpd[ix-1])*(x2m2xi2 + yb*yb)/
					((yb - x)*(yb*yb - xi2));
		...
		{
			// y = -1
			convint += (shift+1.0)*gpd_x*(x2m2xi2 + 1.0)/((-1.0-x)*(1.0-xi2));
		}

		convint *= gamma*c_1_3;

		convint += gpd[ix]*(1.5 + 2.0*log(1.0+x) + 0.5*(x-xi)/xi
				*log((xi-x)*(1.0-xi))
				- 0.5*(x+xi)/xi*log((-xi-x)*(1.0+xi)) );
	...
	if(ix <= 2*n) {
		...
		if(ix == in) {
			convint += ( 2.0*gpd[ix-3] - 9.0*gpd[ix-2] + 18.0*gpd[ix-1]
				- 11.0*gpd[ix] )/(3.0*gamma);
		}

		if((ix-1) == in) {
			double ya  = yy[ix-2];
			double yb  = yy[ix-1];
			double f1 = (shift-ya)*(gpd[ix-2] - gpd_x)*(x-xi)*(ya - xp2xi)/
				(xi*(ya-xi)*(ya-x));
			double f2 = (shift-yb)*(gpd[ix-1] - gpd_x)*(x-xi)*(yb - xp2xi)/
				(xi*(yb-xi)*(yb-x));
			double f3 = ( 2.0*gpd[ix-3] - 9.0*gpd[ix-2]
				+ 18.0*gpd[ix-1] - 11.0*gpd[ix] )/(3.0*gamma);  
			convint += -0.25*f1 + 3.0*f2 + 1.25*f3;
		...
		{
			// y = -1
			convint += -(shift+1.0)*gpd_x*(x-xi)*(-1.0 - xp2xi)/(xi*(-1.0-xi)*(-1.0-x));
		}
		...
		if(ix < (2*n - 2)) {
			...
			for(unsigned int i=ix+2; i < in; i+=2) {
				double y = yy[i];
				double f1 = (shift-y)*(x+xi)*(gpd[i] - gpd_x)*(y - xm2xi)/ (xi*(y+xi)*(y-x));
				convint += 2.0*f1;
			}
		...
		// TODO: Unclear.
		{
			...
			double f1 = (shift-y)*(x+xi)*(gpd[in] - gpd_x)*(y - xm2xi)
					/(xi*(y+xi)*(y-x));
			if((2*n) == in) convint += f1;
			else {
				double ya = yy[2*n-2];
				double f1a = (shift-ya)*(x+xi)*(gpd[2*n-2] - gpd_x)
						*(ya - xm2xi)/(xi*(ya+xi)*(ya-x));
				double f1b = shift*(x+xi)*(gpd[2*n] - gpd_x)*xm2xi/(xi2*x);
		...
		}
		{
			...
			convint +=  ( -11.0*gpd[ix] + 18.0*gpd[ix+1]
				- 9.0*gpd[ix+2] + 2.0*gpd[ix+3] )/(3.0*gamma);
		}
	}

		if((2*n - 2) == ix) {
			double y = yy[2*n-1];
			double f1 = ( - 2.0*gpd[2*n-3] - 3.0*gpd[2*n-2] + 6.0*gpd[2*n-1]
				- gpd[2*n] )/(3.0*gamma);
			double f2 = (shift-y)*(x+xi)*(gpd[2*n-1] - gpd_x)*(y - xm2xi)
					/(xi*(y+xi)*(y-x));
			double f3 =  shift*(gpd[2*n] - gpd_x)*xm2xi*(x+xi)/(xi2*x);
			convint += f1 + 4.0*f2 + f3;
		}

		if((2*n - 1) == ix) {
			double y = yy[2*n-2];
			double f1 = (shift-y)*(x+xi)*(gpd[2*n-2] - gpd_x)*(y - xm2xi)
					/(xi*(y+xi)*(y-x));
		...
		if(ix < 2*n) {
			convint += shift*(gpd[2*n] - gpd_x)*xm2xi*(x+xi)/(xi2*x);
		}
		else {
			convint +=  ( - 11.0*gpd[2*n] + 18.0*gpd[2*n+1]
				-  9.0*gpd[2*n+2] + 2.0*gpd[2*n+3])/(3.0*gamma);
		}
		...
		convint += gpd[ix]*(3.0 + 2.0*log((1.0-x*x)/(1.0+xi))
				+ (x-xi)/xi*log(xi-x)
				- (x+xi)/xi*log(xi+x));
		convint *= m_CF*0.5;
		return convint;
	...
	if(ix < 3*n) {
		...
		if(ix > (2*n+2)) {
			...
			if((ix-1) == in) {
				...
				double f2 =  (shift+yb)*(gpd[ix-1] - gpd_x)*(x-xi)*(yb - xp2xi)
						/(xi*(yb-xi)*(yb-x));
				double f3 = ( 2.0*gpd[ix-3] - 9.0*gpd[ix-2]+18.0*gpd[ix-1]
				- 11.0*gpd[ix] )/(3.0*gamma);
				convint += -0.25*f1 + 3.0*f2 +1.25*f3;
			}
			...
			{
				// y = 0
				convint += -shift*(gpd[2*n] - gpd_x)*(x-xi)*xp2xi/(xi2*x);
			}
		}

		if((2*n + 1) == ix) {
			double y = yy[2*n+2];
			double f1 = -shift*(gpd[2*n] - gpd_x)*(x-xi)*xp2xi/(xi2*x);
			double f2 = ( 2.0*gpd[ix-1] + 3.0*gpd[ix]
				- 6.0*gpd[ix+1] + gpd[ix+2] )/(3.0*gamma);
			double f3 = (shift+y)*(gpd[2*n+2] - gpd_x)*(x-xi)*(y - xp2xi)
					/(xi*(y-xi)*(y-x));
		...
		if((2*n + 2) == ix) {
			double y = yy[2*n+1];
			double f1 = -shift*(gpd[2*n] - gpd_x)*(x-xi)*xp2xi/(xi2*x);
			double f2 = (shift+y)*(gpd[2*n+1] - gpd_x)*(x-xi)*(y - xp2xi)
					/(xi*(y-xi)*(y-x));
			double f3 = ( 2.0*gpd[ix-1] + 3.0*gpd[ix]
				- 6.0*gpd[ix+1] + gpd[ix+2] )/(3.0*gamma);
			convint += 1.25*f1 + 2.0*f2 -0.25*f3;
		}
	...
	if(3*n == ix) {
		...
		// TODO: Suppress block.
		{
			...
			if((4*n) == in) convint += f1;
				...
				double ya = yy[4*n-2];
				double f1a = (shift+ya)*(gpd[4*n-2] - gpd_x)/(ya-xi);
		...
		// TODO: Suppress block.
		{
			// y = x
			convint += ( -11.0*gpd[ix] + 18.0*gpd[ix+1]
			- 9.0*gpd[ix+2] + 2.0*gpd[ix+3] )/(6.0*gamma);
		}

		convint = gamma*convint*c_1_3 + gpd_x*(1.5 + log(0.5*(1.0-xi)/xi));
	...
	if(ix < (4*n-2)) {
		...
		// TODO: Suppress block.
		{
			...
			if((4*n) == in) convint += f1;
				...
				double ya = yy[4*n-2];
				double f1a = (shift+ya)*(gpd[4*n-2] - gpd_x)*(ya*ya + x2m2xi2)/
						((ya - x)*(ya*ya - xi2));
		...
		convint = gamma*convint*c_1_3+ gpd[ix]*(3.0/2.0 + 2.0*log(1.0-x)
				+ 0.5*(x-xi)/xi*log((x-xi)*(1.0+xi))
				- 0.5*(x+xi)/xi*log((xi+x)*(1.0-xi)) );
	...
	printf("The convolution code finished weird.\n Looks like ix is not in the right range (-1,1)\n");
	return 0.;
}

double VinnikovEvolQCDModel::convolSingletqqKernel(const unsigned int ix, const double xi,
		const int n, const double* const gpd, const double* const yy) {
 ...
 //----------------   Ok, lets start  -------------------
	...
	double const c_1_3 = 1.0/3.0;
	double const shift = xi2/(1.0-2.0*xi);
	double const gamma = log(0.5+0.5/xi*sqrt(9.0*xi2-12.0*xi+4.0))/((double)n);
	...
	if((2*n-1) == ix) {
		...
		double f1 = (shift+y)*(gpd[2*n-2] - gpd[ix])*(x2m2xi2 + y2)
				/((y-x)*(y2 - xi2));
		double f2 = (gpd[2*n-3] - 6.0*gpd[2*n-2] + 3.0*gpd[2*n-1])/(3.0*gamma);
		double f3 = -(shift+1.0)*gpd[ix]*(x2m2xi2 + 1.0)/((1.0-x)*(1.0 - xi2));
		return m_CF*( c_1_3*gamma*(-0.25*f1 + 2.0*f2 + 1.25*f3)
				+gpd[ix]*(1.5 + 2.0*log(1.0-x) + (x-xi)/(2.0*xi)
				*log((x-xi)*(1.0+xi)) - (x+xi)/(2.0*xi)*log((x+xi)*(1.0-xi))) );
	}
	...
	if((2*n-2) == ix) {
		...
		double f1 = (-2.0*gpd[2*n-3] - 3.0*gpd[2*n-2] + 6.0*gpd[2*n-1])
				/(3.0*gamma);
		double f2 = (shift+y)*(gpd[2*n-1] - gpd[ix])*(x2m2xi2 + y2)/((y-x)
				*(y2 - xi2));
		double f3 = -(shift+1.0)*gpd[ix]*(x2m2xi2 + 1.0)/((1.0-x)*(1.0 - xi2));
		return m_CF*(c_1_3*gamma*(f1 + 4.0*f2 + f3)+gpd[ix]*(1.5
				+ 2.0*log(1.0-x) + (x-xi)/(2.0*xi)*log((x-xi)*(1.0+xi))
	...
	if(ix < n) {
		...
		if(1 == ix) {
			...
			double f1 = (shift+x)*( -2.0*(xi+x)*x*(gpd[1]-gpd[n])/(xi2-x2) )
						- 2.0*xi*(-3.0*gpd[1] + 6.0*gpd[2] - 1.0*gpd[3])/(6.0*gamma);
			double f2 = (shift+y)*( -2.0*(xi+x)*y*(gpd[2]-gpd[n])/(xi2-y2)
		...
		if(2 == ix) {
			...
			double f1 = (shift+y)*( -2.0*(xi+x)*y*(gpd[1]-gpd[n])/(xi2-y2)
					- 4.0*xi*y*(gpd[1]-gpd_x)/(y2-x2) );
			double f2 =(shift+x)*( -2.0*(xi+x)*x*(gpd[ix]-gpd[n])/(xi2-x2) )
					- 2.0*xi*(-6.0*gpd[1] + 3.0*gpd[2] + 2.0*gpd[3])/(6.0*gamma);
			convint += 4.0*f1 + f2;
		}
		...
		if(ix > 2) {
			...
			for(unsigned int i=2;i<in;i+=2) {
				double y = yy[i];
				double y2 = y*y;
				double f1 =(shift+y)*( -2.0*(xi+x)*y*(gpd[i]-gpd[n])/(xi2-y2)
			...
			if(ix == in) {
				convint +=(shift+x)*( -2.0*(xi+x)*x*(gpd[ix]-gpd[n])/(xi2-x2) )
						- 2.0*xi*(-2.0*gpd[ix-3] + 9.0*gpd[ix-2] - 18.0*gpd[ix-1]
						+ 11.0*gpd[ix])/(6.0*gamma);
			}
			...
			if((ix-1) == in) {
				...
				double yb = yy[ix-1];
				double yb2 = yb*yb;
				double f1 = (shift+ya)*( -2.0*(xi+x)*ya*(gpd[ix-2]-gpd[n])/(xi2-ya2)
						- 4.0*xi*ya*(gpd[ix-2]-gpd_x)/(ya2-x2) );
				double f2 = (shift+yb)* ( -2.0*(xi+x)*yb*(gpd[ix-1]-gpd[n])/(xi2-yb2)
						- 4.0*xi*yb*(gpd[ix-1]-gpd_x)/(yb2-x2) );
				double f3 = (shift+x)* ( -2.0*(xi+x)*x*(gpd[ix]-gpd[n])/(xi2-x2) )
						- 2.0*xi*(-2.0*gpd[ix-3] + 9.0*gpd[ix-2] - 18.0*gpd[ix-1]
						+ 11.0*gpd[ix])/(6.0*gamma);
				convint += -0.25*f1 + 3.0*f2 + 1.25*f3;
			}
		...
		for(unsigned int i=i0+2;i<2*n;i+=2) {
			double y = yy[i];
			double f1 = (shift+y)*( 2.0*x*(gpd[i]-gpd_x)/(xi+y)
		...
		{
			convint += (shift+1.0)*(2.0*x*(-gpd_x)/(xi+1.0)
					+ 4.0*xi*x*(-gpd_x)/(1.0-x2));
		}

		if(ix == i0) {
			convint += 2.0*xi*(-11.0*gpd[ix] + 18.0*gpd[ix+1] - 9.0*gpd[ix+2]
		...
		if((ix+1) == i0) {
			double ya = yy[ix+1];
			double yb = yy[ix+2];
			double f0 = 2.0*xi*(-11.0*gpd[ix] + 18.0*gpd[ix+1] - 9.0*gpd[ix+2]
			+ 2.0*gpd[ix+3])/(6.0*gamma);
			double f1 = (shift+ya)*(2.0*x*(gpd[ix+1]-gpd_x)/(xi+ya)
					+ 4.0*xi*x*(gpd[ix+1]-gpd_x)/(ya*ya-x2));
			double f2 = (shift+yb)*(2.0*x*(gpd[ix+2]-gpd_x)/(xi+yb)
					+ 4.0*xi*x*(gpd[ix+2]-gpd_x)/(yb*yb-x2));
			convint += 1.25*f0 + 3.0*f1 - 0.25*f2;
		}

		convint *= c_1_3*gamma;
		convint += 2.0*xi*gpd[ix]*(log((1.0-x)*x2/(xi2*(1.0+x))) + 1.5);
	...
	if(ix == n) {
...

		{
			...
			if((2*n-1) == in) {
				double ya = yy[2*n-2];
				double f1a = (shift+ya)*(gpd[2*n-2] - gpd_n)/(ya-xi);
	...
	if(ix < (2*n-2)) {
		...
		for(unsigned int i=ix+2; i < in; i+=2) {
			double y = yy[i];
			double y2 = y*y;
			double f1 = (shift+y)*(gpd[i] - gpd_x)
					*(x2m2xi2 + y2)/((y-x)*(y2 - xi2));
			convint += 2.0*f1;
		}
...

		{
			double y = yy[in];
			double y2 = y*y;
			double f1 = (shift+y)*(gpd[in] - gpd_x)
					*(x2m2xi2 + y2)/((y-x)*(y2 - xi2));
			convint += f1;
			...
			if((2*n-1) == in) {
				...
				double f1b = -(shift+1.0)*gpd_x
						*(x2m2xi2 + 1.0)/((1.0-x)*(1.0 - xi2));
				convint += 2.0*f1 - 0.25*f1a + 1.25*f1b;
			}
		}
		...
		convint *= gamma*c_1_3;
		convint += gpd[ix]*(1.5 + 2.0*log(1.0-x) + (x-xi)/(2.0*xi)
				*log((x-xi)*(1.0+xi)) - (x+xi)/(2.0*xi)*log((x+xi)*(1.0-xi)) );
		convint *= m_CF;
...
double VinnikovEvolQCDModel::convolSingletggKernel(const unsigned int ix, const double xi,
		const int n, const double* const gpd, const double* const yy) {
	...
	if((2*n-1) == ix) {
		...
		double f1 = (shift+y)*(gpd[2*n-2] - gpd[ix])
				*(xi2mx2*(xi2-y2) + (x2+y2)*(x-y)*(x-y))
				/((xi2-y2)*(xi2-y2)*(y-x));
		double f2 = (gpd[2*n-3] - 6.0*gpd[2*n-2] + 3.0*gpd[2*n-1])/(6.0*gamma);
		double f3 = -(shift+1.0)*gpd[ix]*(xi2mx2*(xi2-1.0) + (x2+1.0)*(x-1.0)
				*(x-1.0))/((xi2-1.0)*(xi2-1.0)*(1.0-x));
		return 2.0*m_CA*( c_1_3*gamma*(-0.25*f1 + 2.0*f2 + 1.25*f3)
				+ gpd[ix]* ( 2.0*(x2+xi2)*(1.0-x)/(xi*xi2 - xi)
						+ (2.0*xi-x)*(xi+x)*(xi+x)/xi2*log((1.0+xi)/(x+xi))
						- 2.0*xi*log((1.0-xi2)/((1.0-x)*(1.0-x)))
						+ (2.0*xi+x)*(xi-x)*(xi-x)/xi2*log((1.0-xi)/(x-xi))
	...
	if((2*n-2) == ix) {
		...
		double f1 = (-2.0*gpd[2*n-3] - 3.0*gpd[2*n-2] + 6.0*gpd[2*n-1])
				/(6.0*gamma);
		double f2 = (shift+y)*(gpd[2*n-1] - gpd[ix])*(xi2mx2*(xi2-y2)
				+ (x2+y2)*(x-y)*(x-y))/((xi2-y2)*(xi2-y2)*(y-x));
		double f3 = -(shift+1.0)*gpd[ix]*(xi2mx2*(xi2-1.0) + (x2+1.0)*(x-1.0)
				*(x-1.0))/((xi2-1.0)*(xi2-1.0)*(1.0-x));
		return 2.0*m_CA*( c_1_3*gamma*(f1 + 4.0*f2 + f3) + gpd[ix]* ( 2.0
				*(x2+xi2)*(1.0-x)/(xi*xi2 - xi) + (2.0*xi-x)*(xi+x)*(xi+x)/xi2
				*log((1.0+xi)/(x+xi)) - 2.0*xi*log((1.0-xi2)/((1.0-x)*(1.0-x)))
				+ (2.0*xi+x)*(xi-x)*(xi-x)/xi2*log((1.0-xi)/(x-xi))
				+ 14.0/3.0*xi )*0.25/xi )
				+ ( - 0.5*m_CA - 2.0*c_1_3*m_TR)*gpd[ix];
	}
	...
	if(ix < n) {
		...
		if(1 == ix) {
			...
			double f0 = shift*( (gpd[0]-gpd_n)/(xi2*xi2)*one_xipx2*
					( xi2mx2_2*sxi2p4xix - fxi*txi2mx2*xi2x ) + (gpd[0]-gpd_x)
					*fxi*one_xipx2*xi2x/x2 );
			double f1 = (shift+x)*(gpd[1]-gpd_n)/((xi2-x2)*(xi2-x2))*one_xipx2
					* ( xi2mx2_2*(sxi2p4xix - 2.0*x2) - fxi*(txi2mx2-x2)
					*(xi2x+txipx*x2) ) - 2.0*xi*(-2.0*gpd[0] - 3.0*gpd[1]
					+ 6.0*gpd[2] - 1.0*gpd[3])/(6.0*gamma) ;
			double f2 = (shift+y)*( (gpd[2]-gpd_n)/((xi2-y2)*(xi2-y2))*one_xipx2
					*( xi2mx2_2*(sxi2p4xix - 2.0*y2) - fxi*(txi2mx2-y2)
					*(xi2x+txipx*y2) ) - (gpd[2]-gpd_x)*fxi*one_xipx2
					*(xi2x+txipx*y2)/(y2-x2) );
		...
		if(2 == ix) {
			...
			double f1 = (shift+y)*( (gpd[1]-gpd_n)/((xi2-y2)*(xi2-y2))*one_xipx2
					*( xi2mx2_2*(sxi2p4xix - 2.0*y2) - fxi*(txi2mx2-y2)
					*(xi2x+txipx*y2) ) - (gpd[1]-gpd_x)*fxi*one_xipx2
					*(xi2x+txipx*y2)/(y2-x2) );
			double f2 = (shift+x)*(gpd[2]-gpd_n)/((xi2-x2)*(xi2-x2))*one_xipx2
					* ( xi2mx2_2*(sxi2p4xix - 2.0*x2) - fxi*(txi2mx2-x2)
					*(xi2x+txipx*x2) ) - 2.0*xi*(gpd[0] - 6.0*gpd[1]
					+ 3.0*gpd[2] + 2.0*gpd[3])/(6.0*gamma) ;
		...
		if(ix > 2) {
			...
			for(unsigned int i=i0+2;i<ix;i+=2) {
				double y = yy[i];
				double y2 = y*y;
				double f1 = (shift+y)*( (gpd[i]-gpd_n)/((xi2-y2)*(xi2-y2))
						*one_xipx2*( xi2mx2_2*(sxi2p4xix - 2.0*y2)
						- fxi*(txi2mx2-y2)*(xi2x+txipx*y2) )
						- (gpd[i]-gpd_x)*fxi*one_xipx2*(xi2x+txipx*y2)
						/(y2-x2) );
				convint += 2.0*f1;
			}
			...
			for(unsigned int i=i0+1;i<ix;i+=2) {
				...
				double f1 = (shift+y)*( (gpd[i]-gpd_n)/((xi2-y2)*(xi2-y2))
						*one_xipx2*( xi2mx2_2*(sxi2p4xix - 2.0*y2)
						- fxi*(txi2mx2-y2)*(xi2x+txipx*y2) ) - (gpd[i]-gpd_x)
						*fxi*one_xipx2*(xi2x+txipx*y2)/(y2-x2) );
				convint += 4.0*f1;
			}

			{
				// y = x
				convint += (shift+x)*(gpd_x-gpd_n)/((xi2-x2)*(xi2-x2))*one_xipx2
						* ( xi2mx2_2*(sxi2p4xix - 2.0*x2) - fxi*(txi2mx2-x2)
						*(xi2x+txipx*x2) ) - 2.0*xi*(-2.0*gpd[ix-3]
						+ 9.0*gpd[ix-2] - 18.0*gpd[ix-1] + 11.0*gpd[ix])
						/(6.0*gamma) ;
			}
			...
			if(1 == i0) {
				...
				double f0 = shift*( (gpd[0]-gpd_n)/(xi2*xi2)*one_xipx2
						*( xi2mx2_2*sxi2p4xix - fxi*txi2mx2*xi2x )
						+ (gpd[0]-gpd_x)*fxi*one_xipx2 *xi2x/x2 );
				double f1 = (shift+ya)*( (gpd[1]-gpd_n)/((xi2-ya2)*(xi2-ya2))
						*one_xipx2*( xi2mx2_2*(sxi2p4xix - 2.0*ya2) - fxi
						*(txi2mx2-ya2)*(xi2x+txipx*ya2) ) - (gpd[1]-gpd_x)*fxi
						*one_xipx2*(xi2x+txipx*ya2)/(ya2-x2) );
				double f2 = (shift+yb)*( (gpd[2]-gpd_n)/((xi2-yb2)*(xi2-yb2))
						*one_xipx2* ( xi2mx2_2*(sxi2p4xix - 2.0*yb2)
						- fxi*(txi2mx2-yb2)*(xi2x+txipx*yb2) ) - (gpd[1]-gpd_x)
						*fxi*one_xipx2*(xi2x+txipx*yb2)/(yb2-x2) );
				convint += 1.25*f0 + 2.0*f1 - 0.25*f2;
			}

			{
				...
				double y = yy[i0];
				double y2 = y*y;
				convint += (shift+y)*( (gpd[i0]-gpd_n)/((xi2-y2)*(xi2-y2))
						*one_xipx2*( xi2mx2_2*(sxi2p4xix - 2.0*y2) - fxi
						*(txi2mx2-y2)*(xi2x+txipx*y2) ) - (gpd[i0]-gpd_x)
						*fxi*one_xipx2*(xi2x+txipx*y2)/(y2-x2) );
			}
		...
		for(unsigned int i=i0+2;i<2*n;i+=2) {
			double y = yy[i];
			double f1 = (shift+y)/((xi+y)*(xi+y)*(y*y-x2))*(gpd[i]-gpd_x)
					*( (sxi2m2x2 + fxi*y)*(y*y-x2) + fxi*(xi2px2*y + txix2) );
			convint += 2.0*f1;
		}
		...
		{
			// y = 1
			double f1 = -(shift+1.0)/((xi+1.0)*(xi+1.0)*(1.0-x2))*gpd_x
		...
		if(ix == i0) {
			if(x > 0.95*xi) {
				double ya = yy[ix+1];
				double f1 = (shift+ya)/((xi+ya)*(xi+ya)*(ya*ya-x2))
						*(gpd[ix+1]-gpd_x)*( (6.0*xi2 + 4.0*xi*ya - 2.0*x2)
						*(ya*ya-x2)+4.0*xi*((xi2+x2)*ya+2.0*xi*x2) );
				convint += f1;
			}
			else {
				convint += 2.0*xi*(-11.0*gpd[ix] + 18.0*gpd[ix+1] - 9.0*gpd[ix+2]
		...
		if((ix+1) == i0) {
			...
			if(x > 0.95*xi) {
				f0 = (shift+ya)/((xi+ya)*(xi+ya))*4.0*xi*((xi2+x2)*ya+2.0*xi*x2)
						*(gpd[ix+1]-gpd_x)/(ya*ya-x2) ;
			}
			else {
				f0 = 2.0*xi*(-11.0*gpd[ix] + 18.0*gpd[ix+1] - 9.0*gpd[ix+2]
				+ 2.0*gpd[ix+3])/(6.0*gamma);
			}
			double f1 = (shift+ya)/((xi+ya)*(xi+ya))*( (6.0*xi2 + 4.0*xi*ya
					- 2.0*x2)*(gpd[ix+1]-gpd_x)+4.0*xi*((xi2+x2)*ya+2.0*xi*x2)
					*(gpd[ix+1]-gpd_x)/(ya*ya-x2));
			double f2 = (shift+yb)/((xi+yb)*(xi+yb))*( (6.0*xi2 + 4.0*xi*yb
					- 2.0*x2)*(gpd[ix+2]-gpd_x) +4.0*xi*((xi2+x2)*yb+2.0*xi*x2)
					*(gpd[ix+2]-gpd_x)/(yb*yb-x2));
			convint += 3.0*f1 + 1.25*f0 - 0.25*f2;
		}

		convint *= c_1_3*gamma;
		convint += 2.0*xi*gpd[ix]*(log((1.0-x2)/((xi+1.0)*(xi+1.0)))
				- 2.0/(1.0+xi) + 7.0*c_1_3);
		convint += -fxi*(gpd_n-gpd_x)/(xi+x)*(-0.5*(xi-x)*l_xipx_ximx + x)
				-fxi/((xi+x)*(xi+x))*(gpd_n-gpd_x)*(xi*(x+xi)*l_xipx_ximx
				- x*(2.0*xi+x));
		convint += gpd[n]*((xi-x)*2.0*x/xi + (xi-x)*(xi-x)/xi2*(2.0*xi+x)
				*l_xipx_ximx );
		convint += gpd_x*(2.0*(xi-x)*(1.0-x)/(1.0+xi) + 4.0*xi*log((xi+1.0)
				/(xi+x)));
		convint *= m_CA/(2.0*xi);
	...
	if(ix == n) {
		...
		for(unsigned int i=i0+2; i < 2*n; i+=2) {
			double y = yy[i];
			double f1 = (shift+y)*(gpd[i]-gpd[n])/((xi-y)*(xi+y)*(xi+y))*2.0*y*xi
		...
		{
			// y = 1
			convint += (shift+1.0)*(-gpd[n])/((xi-1.0)*(xi+1.0)*(xi+1.0))*2.0*xi
					+ (shift+1.0)*(-gpd_x)/(1.0-xi) ;
		}

		if(ix == i0) {
			convint += (-11.0*gpd[ix+1] + 18.0*gpd[ix+2] - 9.0*gpd[ix+3]
				+ 2.0*gpd[ix+4])/(12.0*gamma);
		}

		if((ix+1) == i0) {
			double ya = yy[ix+1];
			double yb = yy[ix+2];
			double f0 = (-11.0*gpd[ix+1] + 18.0*gpd[ix+2] - 9.0*gpd[ix+3]
				+ 2.0*gpd[ix+4])/(12.0*gamma);
			double f1 = (shift+ya)*(gpd[ix+1]-gpd[n])/((xi-ya)*(xi+ya)*(xi+ya))*2.0
					*ya*xi+(shift+ya)*(gpd[ix+1]-gpd_x)/(ya-xi) ;
			double f2 = (shift+yb)*(gpd[ix+2]-gpd[n])/((xi-yb)*(xi+yb)*(xi+yb))
					*2.0*yb*xi + (shift+yb)*(gpd[ix+2]-gpd_x)/(yb-xi) ;
			convint += 3.0*f1 - 0.25*f2 + 1.25*f0;
		}

		convint *= gamma*c_1_3;
		convint += gpd[ix]* (  4.0*xi2*(1.0-xi)/(xi*xi2 - xi) + 4.0*xi*log((1.0+xi)
				/(2.0*xi)) - 2.0*xi*log((1.0+xi)/(1.0-xi)) + 14.0*c_1_3*xi  )
				*0.25/xi;
	...
	if(ix < (2*n-2)) {
		...
		for(unsigned int i=ix+2; i < in; i+=2) {
			...
			double y2 = y*y;
			double f1 = (shift+y)*( (gpd[i]-gpd_n)/((xi2-y2)*(xi2-y2))
					*(y*xi2px2+xxi2mx3 - tx*y2) + (gpd[i]-gpd_x)/(y-x)  );
			convint += 2.0*f1;
		}
...

		{
			double y = yy[in];
			double y2 = y*y;
			double f1 = (shift+y)*( (gpd[in]-gpd_n)/((xi2-y2)*(xi2-y2))
					*(y*xi2px2+xxi2mx3 - tx*y2) + (gpd[in]-gpd_x)/(y-x) );
			convint += f1;

			if((2*n-1) == in) {
				double ya = yy[2*n-2];
				double ya2 = ya*ya;
				double f1a = (shift+ya)*( (gpd[2*n-2]-gpd_n)/((xi2-ya2)
						*(xi2-ya2))*(ya*xi2px2+xxi2mx3 - tx*ya2)
						+ (gpd[2*n-2]-gpd_x)/(ya-x)  );
				double f1b = -(shift+1.0)*( gpd_n/((xi2-1.0)*(xi2-1.0))
...

		{
			...
			convint += (shift+x)*(gpd_x-gpd_n)/((xi2-x2)*(xi2-x2))
					*(x*xi2px2+xxi2mx3 - tx*x2) + (-11.0*gpd[ix]
					+ 18.0*gpd[ix+1] - 9.0*gpd[ix+2] + 2.0*gpd[ix+3])
					/(6.0*gamma) ;
		}

		convint *= gamma*c_1_3;
		convint += gpd[ix]*( 2.0*(x2+xi2)*(1.0-x)/(xi*xi2 - xi) + (2.0*xi-x)
				*(xi+x)*(xi+x)/xi2*log((1.0+xi)/(x+xi)) - 2.0*xi*log((1.0-xi2)
				/((1.0-x)*(1.0-x))) + (2.0*xi+x)*(xi-x)*(xi-x)/xi2*log((1.0-xi)
				/(x-xi)) + 14.0*c_1_3*xi )*0.25/xi;
		convint += (gpd[n]-gpd[ix])*log((xi-x)*(xi+1.0)/((xi+x)*(xi-1.0)))
				*(3.0*xi2*x-x2*x)/(4.0*xi2*xi);
		convint += -(gpd[n]-gpd[ix])*(x2+xi2)*(1.0-x)/((xi+x)*(xi+1.0)*2.0*xi);
		convint += -(gpd[n]-gpd[ix])*(x2+xi2)*((x-xi)/(xi2-1.0) + x/(xi+x))
				/(2.0*xi2) ;
		convint *= 2.0*m_CA;
		convint += (- 0.5*m_CA - 2.0*c_1_3*m_TR)*gpd[ix];
...
double VinnikovEvolQCDModel::convolSingletgqKernel(const unsigned int ix, const double xi,
		const int n, const double* const gpd, const double* const yy) {
	...
	if((2*n-1) == ix) {
		double y = yy[2*n-2];
		double y2 = y*y;
		double f1 = gpd[2*n-2]*(xi2-x2-2.0*y2+2.0*x*y)/(xi2-y2);
		return m_CF*c_1_3*gamma*(-0.25*f1*(shift+y) + 2.0*gpd[ix]*(shift+x));
	}

	if((2*n-2) == ix) {
		double y = yy[2*n-1];
		double y2 = y*y;
		double f1 = gpd[2*n-1]*(xi2-x2-2.0*y2+2.0*x*y)/(xi2-y2);
		double f2 = gpd[2*n-2];
		return m_CF*c_1_3*gamma*(4.0*f1*(shift+y) + f2*(shift+x));
	}
	...
	if(ix < n) {
		...
		if(1 == ix) {
			double y = yy[2];
			double f1 = (shift+x)*(gpd[1]-gpd_n)*(xi-x)*x/(xi*(xi+x));
			double f2 = (shift+y)*(gpd[2]-gpd_n)*(xi-x)*(xi-x)*y/(xi*(xi2-y*y));
			convint += 2.0*f1 - 0.25*f2;
		}

		if(2 == ix) {
			double y = yy[1];
			double f1 = (shift+y)*(gpd[1]-gpd_n)*(xi-x)*(xi-x)*y/(xi*(xi2-y*y));
			double f2 = (shift+x)*(gpd[2]-gpd_n)*(xi-x)*x/(xi*(xi+x));
			convint += 4.0*f1 + f2;
		}
		...
		if(ix > 2) {
			...
			for(unsigned int i=2; i < in; i+=2) {
				double y = yy[i];
				double f1 = (shift+y)*(gpd[i]-gpd_n)*(xi-x)*(xi-x)*y/(xi*(xi2-y*y));
				convint += 2.0*f1;
			}
			...
			{
				double y = yy[in];
				double f1 = (shift+y)*(gpd[in]-gpd_n)*(xi-x)*(xi-x)*y/(xi*(xi2-y*y));
				convint += f1;

				if(in == (ix - 1)) {
					double ya = yy[ix-2];
					double f1a = (shift+ya)*(gpd[ix-2]-gpd_n)*(xi-x)*(xi-x)*ya
							/(xi*(xi2-ya*ya));
					double f2 = (shift+x)*(gpd[ix]-gpd_n)*(xi-x)*x/(xi*(xi+x));
...

		{
			...
			if((2*n-1) == in) {
				double ya = yy[2*n-2];
				double f1a = (shift+ya)*gpd[2*n-2]*(2.0 - xi2px2_xi/(xi+ya));
		...
		convint += gpd_n*((xi-x)*(xi-x)/xi*log(xi/(xi+x)) + (xi-x)*(xi-x)*0.5
			/xi*log((xi+x)/(xi-x)));
	...
	if(ix < (2*n-2)) {
		...
		for(unsigned int i=ix+2; i < in; i+=2) {
			double y = yy[i];
			double y2 = y*y;
			double f1 = (shift+y)*(xi2mx2 - 2.0*y2 + two_x*y)*gpd[i]/(xi2-y2);
			convint += 2.0*f1;
		}
...

		{
			double y = yy[in];
			double y2 = y*y;
			double f1 = (shift+y)*(xi2mx2 - 2.0*y2 + two_x*y)*gpd[in]/(xi2-y2);
			convint += f1;
...
double VinnikovEvolQCDModel::convolSingletqgKernel(const unsigned int ix, const double xi,
		const int n, const double* const gpd, const double* const yy) {
	...
	if((2*n-1) == ix) {
		double y = yy[2*n-2];
		double y2 = y*y;
		double f1 = gpd[2*n-2]*(xi2m2x2 - y2 + two_x*y)/((xi2-y2)*(xi2-y2));
		double f2 = gpd[ix]/(xi2-x2);
		return -2.0*m_TR*c_1_3*gamma*(2.0*f2*(shift+x) - 0.25*f1*(shift+y));
	}

	if((2*n-2) == ix) {
		double y = yy[2*n-1];
		double y2 = y*y;
		double f1 = gpd[2*n-1]*(xi2m2x2 - y2 + two_x*y)/((xi2-y2)*(xi2-y2));
		double f2 = gpd[2*n-2]/(xi2-x2);
		return -2.0*m_TR*c_1_3*gamma*(4.0*f1*(shift+y) + f2*(shift+x));
	}
	...
	if(ix < n) {
		...
		if(ix>2) {
			...
			for(unsigned int i=2; i < in; i+=2) {
				double y = yy[i];
				double y2 = y*y;
				double f1 = (shift+y)*(gpd[i] - gpd_n)*(xi432 - xi2mxix*y2)
						/((xi2-y2)*(xi2-y2));
				convint += 2.0*f1;
			}
...

			{
				double y = yy[in];
				double y2 = y*y;
				double f1 = (shift+y)*(gpd[in] - gpd_n)*(xi432 - xi2mxix*y2)
						/((xi2-y2)*(xi2-y2));
				convint += f1;
				...
				if((ix-1) == in) {
					...
					double f1a = (shift+ya)*(gpd[ix-2] - gpd_n)
							*(xi432 - xi2mxix*ya2)/((xi2-ya2)*(xi2-ya2));
					double f1b = (shift+x)*(gpd[ix] - gpd_n)
							*(xi432 - xi2mxix*x2)/((xi2-x2)*(xi2-x2));
					convint += 2.0*f1 - 0.25*f1a + 1.25*f1b;
				}
			}
...

		{
			...
			if((2*n-1) == in) {
				double ya = yy[2*n-2];
				double f1a = (shift+ya)*gpd[2*n-2]*xi*x/((xi+ya)*(xi+ya));
				convint += 2.0*f1 - 0.25*f1a;
			}
		}
		...
		convint *= gamma*c_1_3;
		convint +=  gpd_n*(x2/(xi+x) + (xi2-x2)/2.0/xi*log((xi+x)/(xi-x)));
		convint *= 1.0/xi2;
	...
	if(n == ix) {
...

		{
			...
			if((2*n-1) == in) {
				...
				double f1a = (shift+ya)*(gpd[in+1] - gpd_n)/((xi+ya)*(xi+ya)); // Test HM, 4/11/2014
	...
	if(ix < (2*n-2)) {
		...
		for(unsigned int i=ix+2; i < in; i+=2) {
			double y = yy[i];
			double y2 = y*y;
			double f1 = (shift+y)*(gpd_3n - gpd[i])*(xi2m2x2 - y2 + two_x*y)
					/((xi2-y2)*(xi2-y2));
			convint += 2.0*f1;
		}
...

		{
			double y = yy[in];
			double y2 = y*y;
			double f1 = (shift+y)*(gpd_3n - gpd[in])*(xi2m2x2 - y2 + two_x*y)
					/((xi2-y2)*(xi2-y2));
			convint += f1;
			...
			if((2*n-1) == in) {
				...
				double f1a = (shift+ya)*(gpd_3n - gpd[2*n-2])
						*(xi2m2x2 - ya2 + two_x*ya)/((xi2-ya2)*(xi2-ya2));
		...
		convint *= gamma*c_1_3;
		convint += -gpd_3n*(0.5*(xi2-x2)/(xi2*xi)*log((xi+1.0)*(xi-x)
				/((xi+x)*(xi-1.0))) + (x-x2)/(xi2*(xi2-1.0)));
