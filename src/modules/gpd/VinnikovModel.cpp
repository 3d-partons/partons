...
const unsigned int VinnikovModel::classId = BaseObjectRegistry::getInstance()->registerBaseObject(new VinnikovModel("VinnikovModel"));

const double VinnikovModel::EPS_BETR = 1.E-8;
const int VinnikovModel::VinnikovModel::SIMP_INT = 100;

VinnikovModel::VinnikovModel(const std::string &className) : GPDModule(className) {
...
void VinnikovModel::initModule() {
    ...
    for(int i = 0; i < 6*n_par; i++) param[i] = (i == 0)?(2.):(1.);
...
PartonDistribution VinnikovModel::computeH(){
    ...
    if(m_x > 0.){
        ...
        uVal = gpdh(1, m_x, m_xi, m_t, n_par, param) - gpdh(1, -m_x, m_xi, m_t, n_par, param);
        uSea = gpdh(1,-m_x, m_xi, m_t, n_par, param);

        dVal = gpdh(2, m_x, m_xi, m_t, n_par, param) - gpdh(2, -m_x, m_xi, m_t, n_par, param);
        ...
        uValMx = gpdh(1, -m_x, m_xi, m_t, n_par, param) - gpdh(1, m_x, m_xi, m_t, n_par, param);
        dValMx = gpdh(2, -m_x, m_xi, m_t, n_par, param) - gpdh(2, m_x, m_xi, m_t, n_par, param);
    }
...
PartonDistribution VinnikovModel::computeHt(){
    ...
    if(m_x > 0.){
        ...
        uVal = gpdh_pol(1, m_x, m_xi, m_t, n_par, param) - gpdh_pol(1, -m_x, m_xi, m_t, n_par, param);
        uSea = gpdh_pol(1,-m_x, m_xi, m_t, n_par, param);

        dVal = gpdh_pol(2, m_x, m_xi, m_t, n_par, param) - gpdh_pol(2, -m_x, m_xi, m_t, n_par, param);
...
double VinnikovModel::gpdh(int const i_part, double x, double const xi, double const t, int const n_par, double* const param)
{
  /*
  ...
  */
  ...
  if(x < -1.0)
    {
      if((x+EPS_BETR) >= -1.0) return 0.0;
      printf("gpdh (gpdh.c): YOUR X SEEMS TO BE < -1\n (x = %20.10e )IT IS ERROR\n",x);
      return 0.0;
    }
  ...
  if(x < -xi)
    {
      ...
      double h = dd_int_simp(bmin, bmax, &VinnikovModel::dd1, i_part, x, xi, t, n_par, param);
      return h/xi;
    }
  ...
  if(x <= 0.0)
    {
      ...
      double h1 = dd_int_simp(0.0, bmax1, &VinnikovModel::dd2, i_part, x, xi, t, n_par, param);
      double h2 = dd_int_simp(bmax1, bmax2, &VinnikovModel::dd1, i_part, x, xi, t, n_par, param);
      return (h1+h2)/xi;
    }
  if(x < xi)
    {
      ...
      double bmax2 = (xi+x)/(1.0+xi);
      double h1 = dd_int_simp(0.0, bmax1, &VinnikovModel::dd2, i_part, x, xi, t, n_par, param);
      double h2 = dd_int_simp(bmax1, bmax2, &VinnikovModel::dd3, i_part, x, xi, t, n_par, param);
      return (h1+h2)/xi;
    }
  if(x <= 1.0)
    {
      double bmin = (x-xi)/(1.0-xi);
      double bmax = (x+xi)/(1.0+xi);
      double h = dd_int_simp(bmin, bmax, &VinnikovModel::dd3, i_part, x, xi, t, n_par, param);
      return h/xi;
    }
  if((x-EPS_BETR) <= 1.0) return 0.0;
  printf("gpdh (gpdh.c): YOUR X SEEMS TO BE > 1\n (x = %20.10e )IT IS ERROR\n",x);
...
double VinnikovModel::dd1(int const i_part, double const beta, double const x, double const xi, double const t, int const n_par, double* const param)
{
  double prof_b;
  double alpha = (x+beta)/xi;
  if(0 == i_part) prof_b = param[0];
  else prof_b = param[n_par*(i_part+2)];
  ...
  if(0 == i_part) return  pf*glu(beta, t);
  if(1 == i_part) return -pf*usea(beta, t);
  if(2 == i_part) return -pf*dsea(beta, t);
  if(3 == i_part) return -pf*ssea(beta, t);
...
double VinnikovModel::dd2(int const i_part, double const beta, double const x, double const xi, double const t, int const n_par, double* const param)
{
  ...
  if(0 == i_part) return (pf2_val+pf1_val)*glu(beta,t);
  
  if(1 == i_part) return (pf2_sea-pf1_sea)*usea(beta,t)
		    + pf2_val*uval(beta,t);
  
  if(2 == i_part) return (pf2_sea-pf1_sea)*dsea(beta,t)
		    + pf2_val*dval(beta,t);
  
  if(3 == i_part) return (pf2_sea-pf1_sea)*ssea(beta,t);
...
double VinnikovModel::dd3(int const i_part, double const beta, double const x, double const xi, double const t, int const n_par, double* const param)
{
  double prof_b_val = param[n_par*i_part];
  double prof_b_sea = param[n_par*(i_part+2)];
  ...
  if(0 == i_part) return pf_val*glu(beta, t);

  if(1 == i_part) return pf_val*uval(beta, t)
		   + pf_sea*usea(beta, t);

  if(2 == i_part) return pf_val*dval(beta, t)
		   + pf_sea*dsea(beta, t);

  if(3 == i_part) return pf_sea*ssea(beta, t);
...
double VinnikovModel::uval(double const x, double const t)
{
  ...
  double fq = - alp*(1.0-x)*(1.0-x)*(1.0-x)*log(x)
    + B_q*(1.0-x)*(1.0-x)*(1.0-x) + A_q*x*(1.0-x)*(1.0-x);
  ...
  return a0*exp(log(x)*a1 + log(1.0-x)*a2 + a3*x + log(1.0 + a4*x)*a5 + t*fq);
}
...
double VinnikovModel::usea(double const x, double const t)
{
  ...
  return a0*exp(log(x)*a1 + log(1.0-x)*a2 + a3*x + log(1.0 + a4*x)*a5)
...
double VinnikovModel::glu(double const x, double const t)
{
  ...
  return a0*exp(log(x)*a1 + log(1.0-exp(2.0/3.0*log(x)))*a2 + a3*sqrt(x)
		+ log(1.0 + a4*x)*a5);
}
...
double VinnikovModel::gpdh_pol(int const i_part, double x, double const xi, double const t, int const n_par, double* const param)
{
  if(x==xi) x -= EPS_BETR;
  if(x==-xi) x += EPS_BETR;
  ...
  if(x < -1.0)
    {
      if((x+EPS_BETR) >= -1.0) return 0.0;
      printf("gpdh_pol (gpdh_pol.c): YOUR X SEEMS TO BE < 1\n (x = %20.10e )IT IS ERROR\n",x);
      return 0.0;
    }
  ...
  if(x < -xi)
    {
      double bmin = (-x-xi)/(1.0-xi);
      double bmax = (-x+xi)/(1.0+xi);
      double h = dd_int_simp(bmin, bmax, &VinnikovModel::dd1_pol, i_part, x, xi, t, n_par, param);
      return h/xi;
    }
  ...
  if(x <= 0.0)
    {
      double bmax1 = (xi+x)/(1.0+xi);
      double bmax2 = (xi-x)/(1.0+xi);
      double h1 = dd_int_simp(0.0, bmax1, &VinnikovModel::dd2_pol, i_part, x, xi, t, n_par, param);
      double h2 = dd_int_simp(bmax1, bmax2, &VinnikovModel::dd1_pol, i_part, x, xi, t, n_par, param);
      return (h1+h2)/xi;
    }
  if(x < xi)
    {
      ...
      double bmax2 = (xi+x)/(1.0+xi);
      double h1 = dd_int_simp(0.0, bmax1, &VinnikovModel::dd2_pol, i_part, x, xi, t, n_par, param);
      double h2 = dd_int_simp(bmax1, bmax2, &VinnikovModel::dd3_pol, i_part, x, xi, t, n_par, param);
      return (h1+h2)/xi;
    }
  if(x <= 1.0)
    {
      double bmin = (x-xi)/(1.0-xi);
      double bmax = (x+xi)/(1.0+xi);
      double h = dd_int_simp(bmin, bmax, &VinnikovModel::dd3_pol, i_part, x, xi, t, n_par, param);
      return h/xi;
    }
  if((x-EPS_BETR) <= 1.0) return 0.0;
  printf("gpdh_pol (gpdh_pol.c): YOUR X SEEMS TO BE > 1\n (x = %20.10e )IT IS ERROR\n",x);
...
double VinnikovModel::dd1_pol(int const i_part, double const beta, double const x, double const xi, double const t, int const n_par, double* const param)
{
  double alpha = (x+beta)/xi;
  ...
  if(alpha >= (1.0-beta))
    {
      if((alpha+beta-1.0) > EPS_BETR)
	printf("dd1_pol (gpdh_pol.c): WARNING1, beta = %15.7e, alpha = %15.7e\n",beta,alpha);
      return 0.0;
    }
  if(alpha <= (-1.0+beta))
    {
      if((alpha-beta+1.0) < (- EPS_BETR))
	printf("dd1_pol (gpdh_pol.c): WARNING2, beta = %15.7e, alpha = %15.7e\n",beta,alpha);
      return 0.0;
    }
  
  double pf = prof_funct(prof_b,beta,alpha);
  
  if(0 == i_part) return -pf*glu_pol(beta, t);
  if(1 == i_part) return pf*usea_pol(beta, t);
  if(2 == i_part) return pf*dsea_pol(beta, t);
  if(3 == i_part) return pf*ssea_pol(beta, t);
...
double VinnikovModel::dd2_pol(int const i_part, double const beta, double const x, double const xi, double const t, int const n_par, double* const param)
{
  double prof_b_val = param[n_par*i_part];
  double prof_b_sea = param[n_par*(i_part+2)];
  ...
  if(alpha1>=1.0-beta)
    {
      if((alpha1-1.0+beta) > EPS_BETR)
	printf("dd2_pol (gpdh_pol.c): WARNING1, beta = %15.7e, alpha = %15.7e\n",beta,alpha1);
      return 0.0;
    }
  if(alpha1<= -1.0+beta)
    {
      if((alpha1+1.0-beta) < -EPS_BETR)
	printf("dd2_pol (gpdh_pol.c): WARNING2, beta = %15.7e, alpha = %15.7e\n",beta,alpha1);
      return 0.0;
    }
  if(alpha2>=1.0-beta)
    {
      if((alpha2-1.0+beta) > EPS_BETR)
	printf("dd2_pol (gpdh_pol.c): WARNING3, beta = %15.7e, alpha = %15.7e\n",beta,alpha2);
      return 0.0;
    }
  if(alpha2<= -1.0+beta)
    {
      if((alpha2+1.0-beta) < -EPS_BETR)
	printf("dd2_pol (gpdh_pol.c): WARNING4, beta = %15.7e, alpha = %15.7e\n",beta,alpha2);
      return 0.0;
    }
  double pf1_val = prof_funct(prof_b_val,beta,alpha1);
  double pf2_val = prof_funct(prof_b_val,beta,alpha2);
  ...
  if(0 == i_part) return (pf2_val-pf1_val)*glu_pol(beta,t);
  
  if(1 == i_part) return (pf2_sea+pf1_sea)*usea_pol(beta,t)
		    + pf2_val*uval_pol(beta,t);
  
  if(2 == i_part) return (pf2_sea+pf1_sea)*dsea_pol(beta,t)
		    + pf2_val*dval_pol(beta,t);
  
  if(3 == i_part) return (pf2_sea+pf1_sea)*ssea_pol(beta,t);
...
double VinnikovModel::dd3_pol(int const i_part, double const beta, double const x, double const xi, double const t, int const n_par, double* const param)
{
  double prof_b_val = param[n_par*i_part];
  double prof_b_sea = param[n_par*(i_part+2)];
  ...
  if(alpha>= (1.0-beta))
    {
      if((alpha+beta-1.0) > EPS_BETR)
	printf("dd3_pol (gpdh_pol.c): WARNING1, beta = %15.7e, alpha = %15.7e\n",beta,alpha);
      return 0.0;
    }
  if(alpha<= (-1.0+beta))
    {
      if((alpha+1.0-beta) < (-EPS_BETR))
	printf("dd3_pol (gpdh_pol.c): WARNING2, beta = %15.7e, alpha = %15.7e\n",beta,alpha);
      return 0.0;
    }
  ...
  if(0 == i_part) return pf_val*glu_pol(beta, t);

  if(1 == i_part) return pf_val*uval_pol(beta, t)
		   + pf_sea*usea_pol(beta, t);

  if(2 == i_part) return pf_val*dval_pol(beta, t)
		   + pf_sea*dsea_pol(beta, t);

  if(3 == i_part) return pf_sea*ssea_pol(beta, t);
...
double VinnikovModel::uval_pol(double const x, double const t)
{
  ...
  double coeff = (1.0+gam*a/(a+b+1.0))*gammf(a)*gammf(b+1.0)/gammf(a+b+1.0);

  return eta/coeff*exp((a-1.0)*log(x) + b*log(1.0-x) + t*fq)*(1.0+gam*x);
}
...
double VinnikovModel::usea_pol(double const x, double const t)
{
  ...
  return eta/coeff*exp((a-1.0)*log(x) + b*log(1.0-x))*(1.0+gam*x);
}
...
double VinnikovModel::glu_pol(double const x, double const t)
{
  ...
  return x*eta/coeff*exp((a-1.0)*log(x) + b*log(1.0-x))*(1.0+gam*x);
}

// COMMON FUNCTIONS ===================================================================================

double VinnikovModel::prof_funct(double const b, double const beta, double const alpha)
{
...
double VinnikovModel::dd_int_simp(double const bmin, double const bmax, double (VinnikovModel::* dd)(int, double, double, double, double, int, double*), int const i_part, double const x, double const xi, double const t, int const n_par, double* const param)
{
  int i;
  double integr = 0.0;

  double step = log((bmax-EPS_BETR)/(bmin+EPS_BETR))/((double)SIMP_INT);


  if((bmax-bmin) < EPS_BETR)
    return (bmax-bmin)*(this->*dd)(i_part,0.5*(bmax+bmin),x,xi,t,n_par,param);
  ...
  for(i=2;i<SIMP_INT;i+=2)
    {
      ...
      integr += 2.0*beta*step*(this->*dd)(i_part, beta, x, xi, t, n_par, param);
    }

  for(i=1;i<SIMP_INT;i+=2)
    {
      double beta = (bmin+EPS_BETR)*exp(i*step);
      integr += 4.0*beta*step*(this->*dd)(i_part, beta, x, xi, t, n_par, param);
    }
  //  integr += dd(bmin+EPS_BETR,x,xi) + dd(bmax-EPS_BETR,x,xi);
  integr +=
    (bmin+EPS_BETR)*step*(this->*dd)(i_part, bmin+EPS_BETR, x, xi, t, n_par, param) +
    (bmax-EPS_BETR)*step*(this->*dd)(i_part, bmax-EPS_BETR, x, xi, t, n_par, param);
