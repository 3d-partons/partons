...
DVCSCFFHeavyQuarkModel::DVCSCFFHeavyQuarkModel(const std::string& className) :
        DVCSCFFModel(className), m_betas(std::complex<double>(0.0, 0.0)), m_rs(
                ...
                std::complex<double>(0.5, 0.0)), m_eps(1.e-5) {
...
// TODO Add a massless part
    ...
    std::complex<double> GluonNLOA(
            m_betas * std::log(m_rs) - m_betaq * std::log(m_rq));
    GluonNLOA *= (2. * z - 6.) / (z - 1.);
...
std::complex<double> DVCSCFFHeavyQuarkModel::KernelGluonNLOV(double x) {
    ...
    GluonNLOV -= (1.
            + 8. * eta * z * (1. - (1. + 4. * eta) * z) / (z - 1.) / (z - 1.))
            * std::log(m_rs) * std::log(m_rs);
    GluonNLOV += (1.
            - 2. * eta * (1. + (3. + 8. * eta) * z * z) / (z - 1.) / (z - 1.))
            * std::log(m_rq) * std::log(m_rq);
...
//	std::complex<double> beta(1.-4.*mq*mq/(s+m_eps*m_eps)*s, 4.*mq*mq/(s+m_eps*m_eps)*m_eps);
    std::complex<double> beta(
            1. - 4. * mq * mq / (s + std::complex<double>(0.0, m_eps)));
    return std::sqrt(beta);
}

std::complex<double> DVCSCFFHeavyQuarkModel::r(std::complex<double> s,
        double mq) {
