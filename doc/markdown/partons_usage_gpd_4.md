# GPDService {#usage_gpd_4}
 
[TOC]

# generatePlotFile task {#usage_gpd_4_lab}
 
## Task name {#usage_gpd_4_namexml}
...
This task is for the selection of specific data from the database and write them to a file to be used e.g. for plotting purposes. To run this task you must first fill the database with some results. You can use for instance `computeManyKinematicsForGPD` task in XML interface with `storeInDB="1"`. After the successful insertion PARTONS returns to the standard output unique computation_id, which you should use whenever needed to refer the inserted data. 
...
## XML example {#usage_gpd_4_examplexml} 
