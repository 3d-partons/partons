...
%All the external libraries should be availible through the package manager:
...
Visit [this page](@ref download) to download %PARTONS source code. To achieve this, you may need Git command line tool to be available. It can be installed in the following way:
```sh
...
This section is for building the projects without using of any Integrated Development Environment (IDE). Skip it and see [this tutorial](@ref eclipse) if you want to use Eclipse for managing of source code building. 

We will detail the procedure in the case of [elementary-utils](https://drf-gitlab.cea.fr/partons/core/elementary-utils). For other %PARTONS sub-project the procedure is exactly the same.

Go to `build` directory and call CMake in the following way:
```sh
...
Except the main function encoded in `main.cpp`, you may be also interested in the following components: 
* `CMakeLists.txt`: CMake configuration file
* `cmake/Modules/FindXXXX.cmake`: CMake modules for finding of external libraries
